...
param(
  ...
  [string[]]$BackupOnlyRoots,[string[]]$SignerOnlyRoots,[string[]]$SignerStateRoots,[string[]]$AdminOnlyRoots,
...
function Rights([string]$Name,[string]$Identity,[string]$Core,[string]$Edge,[string]$Backup){
  ...
  if($Identity-eq$script:SignerSid){if($Name-in@('BACKUP_RECEIPT','SIGNER_STATE')){return $modify};if($Name-in@('BACKUP_ONLY','SIGNER_ONLY','SHARED_RUNTIME','ADMIN_EVIDENCE')){return $rx}}
  return $null
}
...
$classes=@(
  ...
  [pscustomobject]@{Name='BACKUP_ONLY';Roots=Roots $root $BackupOnlyRoots},[pscustomobject]@{Name='SIGNER_ONLY';Roots=Roots $root $SignerOnlyRoots},[pscustomobject]@{Name='SIGNER_STATE';Roots=Roots $root $SignerStateRoots},[pscustomobject]@{Name='ADMIN_ONLY';Roots=Roots $root $AdminOnlyRoots})
