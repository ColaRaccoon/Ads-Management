...
param(
  ...
  [string[]]$BackupOnlyRoots,[string[]]$AdminOnlyRoots,
...
function Rights([string]$Name,[string]$Identity,[string]$Core,[string]$Edge,[string]$Backup){
  ...
  if($Identity-eq$Backup){if($Name-eq'BACKUP_RECEIPT'){return$modify};if($Name-in@('CORE_MODIFY','BACKUP_ONLY','SHARED_RUNTIME','ADMIN_EVIDENCE')){return$rx}}
  return$null
}
function ExpectedSids($Class,[string]$Core,[string]$Edge,[string]$Backup){$ids=@($systemSid,$administratorsSid);foreach($id in @($Core,$Edge,$Backup)){if($id-and$null-ne(Rights $Class $id $Core $Edge $Backup)){$ids+=$id}};return@($ids|Sort-Object -Unique)}
...
$root=SafeRoot $ApprovedDataRoot;$core=Sid $CoreServiceAccount;$edge=Sid $EdgeServiceAccount;$backupSid=Sid $BackupAccount
if(@(@($core,$edge,$backupSid) | Sort-Object -Unique).Count -ne 3){throw 'ACL_PRINCIPALS_MUST_BE_DISTINCT'}
$classes=@(
  ...
  [pscustomobject]@{Name='BACKUP_ONLY';Roots=Roots $root $BackupOnlyRoots},[pscustomobject]@{Name='ADMIN_ONLY';Roots=Roots $root $AdminOnlyRoots})
...
if($Action-eq'Verify'){foreach($class in $classes){Visit $class {param($p,$n)AssertExact $p $n $core $edge $backupSid}};$digest=Digest $classes;if($EvidenceOutputPath){$classRoots=[ordered]@{};foreach($class in $classes){$classRoots[$class.Name]=@($class.Roots)};[IO.File]::WriteAllText($EvidenceOutputPath,([ordered]@{result='PASS';dataRoot=$root;filesystem='NTFS';nonReparse=$true;leastPrivilege=$true;exactAcl=$true;coreServiceSid=$core;edgeServiceSid=$edge;backupSid=$backupSid;classRoots=$classRoots;descriptorDigest=$digest;completedAt=(Get-Date).ToUniversalTime().ToString('o')}|ConvertTo-Json -Depth 6),(New-Object Text.UTF8Encoding($false)))};[pscustomobject]@{Result='PASS';DescriptorDigest=$digest;BroadAccess=$false;ExactAcl=$true}|ConvertTo-Json;exit 0}
