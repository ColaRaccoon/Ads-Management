...
function TargetFingerprint($Evidence) {
  return Sha256Text -Value (@('7',$Evidence.result,$Evidence.targetType,$Evidence.dataRoot,$Evidence.backupRoot,$Evidence.dataDiskUniqueId,$Evidence.backupDiskUniqueId,$Evidence.nasServer,$Evidence.nasShare,$Evidence.nasServerIdentitySha256,[string]$Evidence.nasResolvedAddressCount,([string]($Evidence.nasLocalAliasRejected-eq$true)).ToLowerInvariant(),([string]($Evidence.nasShareAclAdministrativelyConfirmed-eq$true)).ToLowerInvariant(),$Evidence.backupWriterSid,$Evidence.signerReaderSid,$Evidence.encryptionProof,$Evidence.retentionControl,$Evidence.completedAt) -join "`n")
}
...
if ($evidence.version-ne6-or$evidence.result-ne'PASS'-or$evidence.backupRoot-cne$backupRoot-or$evidence.dataRoot-cne$dataRoot-or-not$evidence.separateReceiptSigner-or-not$evidence.encryptedAtRestOrTransport) { throw 'BACKUP_TARGET_NOT_VERIFIED' }
