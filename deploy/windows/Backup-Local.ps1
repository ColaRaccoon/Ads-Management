...
param(
  ...
  [string]$ExpectedBackupTargetEvidenceSha256,
  [string]$ExpectedBackupTargetFingerprint,
  ...
  [ValidateSet('OneTime','Scheduled')][string]$AuthorizationMode='OneTime',
  ...
  [ValidateSet('RunOneTime')][string]$PlannedAction='RunOneTime',[string]$ApprovedPlanSha256,[string]$ApprovalNonce,[string]$ApprovalIssuedAt,[string]$ApprovalExpiresAt,[string]$ApprovalInstanceId,[string]$ApprovalLedgerPath,[switch]$Approved
)
...
function BackupContractSha256 { return Sha256Text ((BackupContractParameters|ConvertTo-Json -Depth 12 -Compress)) }
function New-BackupApprovalPlan {
  $parameters=BackupContractParameters;$parameters.contractSha256=BackupContractSha256
  return New-ApprovalPlan $PSCommandPath 'RunOneTime' $parameters 'Exact configured Supabase PostgreSQL backup and NTFS storage snapshot' 'Reads the exact approved source and writes one bounded backup only to the exact verified target' 'Remove only the incomplete staging directory; published backups follow retention policy'
}
...
function TargetFingerprint($Evidence) {
  return Sha256Text -Value (@('8',$Evidence.result,$Evidence.targetType,$Evidence.dataRoot,$Evidence.backupRoot,$Evidence.dataDiskUniqueId,$Evidence.backupDiskUniqueId,$Evidence.nasIdentityHelperSha256,$Evidence.nasServer,$Evidence.nasShare,$Evidence.nasServerIdentitySha256,[string]$Evidence.nasResolvedAddressCount,([string]($Evidence.nasLocalAliasRejected-eq$true)).ToLowerInvariant(),([string]($Evidence.nasShareAclAdministrativelyConfirmed-eq$true)).ToLowerInvariant(),$Evidence.backupWriterSid,$Evidence.signerReaderSid,$Evidence.encryptionProof,$Evidence.retentionControl,$Evidence.completedAt) -join "`n")
}
function Assert-CurrentBackupTarget($Evidence,[string]$DataRoot,[string]$Root){
  $system='S-1-5-18';$admin='S-1-5-32-544';$acl=Get-Acl -LiteralPath $Root;$expected=@($system,$admin,[string]$Evidence.backupWriterSid,[string]$Evidence.signerReaderSid)|Sort-Object -Unique
  if(-not$acl.AreAccessRulesProtected-or$acl.Owner.Translate([Security.Principal.SecurityIdentifier]).Value-notin@($system,$admin)-or@($acl.Access).Count-ne4){throw 'BACKUP_TARGET_ACL_DRIFT'}
...
$requiredFiles=@($RuntimeConfigPath,$PgDumpPath,$PsqlPath,$PgPassFile,$BackupIntegrityKeyFile,$ReceiptPublisherPath,$SemanticSignerPath,$nasIdentityHelper,$NodePath,$FileSystemEvidencePath)
...
Assert-PinnedFile $nasIdentityHelper $ExpectedNasIdentityHelperSha256 'NAS_IDENTITY_HELPER_HASH_MISMATCH'
...
$sharedExecutors=@($NodePath,$PsqlPath,$PgDumpPath,$ReceiptPublisherPath,$SemanticSignerPath,$nasIdentityHelper);if($AuthorizationMode-eq'Scheduled'){$sharedExecutors+=$AttestationVerifierPath};if($BackupMode-eq'LOCAL_RELEASE'){$sharedExecutors+=$ReleaseVerifierPath}
...
if($BackupMode-eq'LOCAL_RELEASE'-and($releaseResult.releaseId-cne$config.release.id-or$releaseResult.migrationDigest-cne$config.release.migrationDigest-or$releaseResult.appliedMigrationDigest-cne$config.release.appliedMigrationDigest)){throw 'BACKUP_RELEASE_ID_MISMATCH'}
...
$configuredEvidencePath=[IO.Path]::GetFullPath([string]$config.backup.physicalTargetEvidencePath)
if((Deadline-BoundFileHash $configuredEvidencePath)-cne$ExpectedBackupTargetEvidenceSha256.ToLowerInvariant()-or-not(UnderAny $configuredEvidencePath $fs.classRoots.ADMIN_EVIDENCE)){throw 'BACKUP_TARGET_EVIDENCE_BINDING_REJECTED'}
...
if ($evidence.version-ne7-or$evidence.result-ne'PASS'-or$evidence.backupRoot-cne$backupRoot-or$evidence.dataRoot-cne$dataRoot-or$evidence.nasIdentityHelperPath-ine$nasIdentityHelper-or$evidence.nasIdentityHelperSha256-cne$ExpectedNasIdentityHelperSha256.ToLowerInvariant()-or-not$evidence.separateReceiptSigner-or-not$evidence.encryptedAtRestOrTransport) { throw 'BACKUP_TARGET_NOT_VERIFIED' }
...
if($targetFingerprint-cne$ExpectedBackupTargetFingerprint.ToLowerInvariant()-or$backupRoot-cne$(if($ExpectedBackupRoot.StartsWith('\\')){$ExpectedBackupRoot.TrimEnd('\')}else{[IO.Path]::GetFullPath($ExpectedBackupRoot)})){throw 'BACKUP_TARGET_APPROVAL_MISMATCH'}
...
try {
  ...
  $expectedAppliedDigest=Sha256Text -Value $expectedAppliedCanonical
  if($appliedMigrationCanonical-cne$expectedAppliedCanonical-or($BackupMode-eq'LOCAL_RELEASE'-and($appliedMigrationDigest-cne[string]$config.release.appliedMigrationDigest-or$expectedAppliedDigest-cne[string]$releaseResult.appliedMigrationDigest))){throw 'DATABASE_MIGRATION_CHAIN_DRIFT'}
  Assert-BackupDeadline
  ...
  $manifest = [ordered]@{
    version=6; result='COMPLETE'; backupId=$backupId; createdAt=$createdAt; rpoHours=24; rtoHours=4
    ...
    nodeSha256=$ExpectedNodeSha256.ToLowerInvariant();psqlSha256=$ExpectedPsqlSha256.ToLowerInvariant();pgDumpSha256=$ExpectedPgDumpSha256.ToLowerInvariant();executorSetDigest=$executorSetDigest;filesystemEvidenceSha256=$ExpectedFileSystemEvidenceSha256.ToLowerInvariant();nasIdentityHelperSha256=$ExpectedNasIdentityHelperSha256.ToLowerInvariant()
  }
  Assert-PinnedFile $BackupIntegrityKeyFile $ExpectedBackupIntegrityKeySha256 'BACKUP_INTEGRITY_KEY_HASH_MISMATCH'
  ...
  foreach($incompleteArtifact in @($staging,$final)){if($incompleteArtifact-and(Test-Path -LiteralPath $incompleteArtifact)){$cleanupFailed=$true}}
