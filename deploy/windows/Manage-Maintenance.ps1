...
param(
  ...
  [string]$InternalProbeTokenFile,[string]$ExpectedInternalProbeTokenSha256,
  ...
  [ValidateSet('Enable','Disable')][string]$PlannedAction,[string]$ApprovedPlanSha256,[string]$ApprovalNonce,[string]$ApprovalIssuedAt,[string]$ApprovalExpiresAt,[string]$ApprovalInstanceId,[string]$ApprovalLedgerPath,[switch]$Approved
)
...
if(-not(Test-Path -LiteralPath $InternalProbeTokenFile -PathType Leaf)){throw 'INTERNAL_PROBE_TOKEN_FILE_REQUIRED'};AssertPinned $InternalProbeTokenFile $ExpectedInternalProbeTokenSha256 'INTERNAL_PROBE_TOKEN_HASH_MISMATCH'|Out-Null
