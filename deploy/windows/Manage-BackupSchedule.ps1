...
param(
  ...
  [string]$PublisherBrokerPath,[string]$ExpectedPublisherBrokerSha256,
  ...
  [string]$SignerAccount,
  [PSCredential]$SignerCredential,
  [string]$BackupReceiptPrivateKeyPath,[string]$SignerScratchRoot,[string]$SignerReplayLedgerRoot,
...
function ExpectedPublisherArguments {
  return @(
    '-NoProfile','-NonInteractive','-ExecutionPolicy','RemoteSigned','-File',(Q $PublisherBrokerPath),'-ExpectedPublisherBrokerSha256',$ExpectedPublisherBrokerSha256,
    '-PowerShellPath',(Q $PowerShellPath),'-ExpectedPowerShellSha256',$ExpectedPowerShellSha256,'-RuntimeConfigPath',(Q $RuntimeConfigPath),'-ExpectedRuntimeConfigSha256',$ExpectedRuntimeConfigSha256,'-FileSystemEvidencePath',(Q $FileSystemEvidencePath),'-ExpectedFileSystemEvidenceSha256',$ExpectedFileSystemEvidenceSha256,
    '-NodePath',(Q $NodePath),'-ExpectedNodeSha256',$ExpectedNodeSha256,'-ReceiptPublisherPath',(Q $ReceiptPublisherPath),'-ExpectedReceiptPublisherSha256',$ExpectedReceiptPublisherSha256,'-SemanticSignerPath',(Q $SemanticSignerPath),'-ExpectedSemanticSignerSha256',$ExpectedSemanticSignerSha256,'-PgRestorePath',(Q $PgRestorePath),'-ExpectedPgRestoreSha256',$ExpectedPgRestoreSha256,
    '-BackupIntegrityKeyFile',(Q $BackupIntegrityKeyFile),'-ExpectedBackupIntegrityKeySha256',$ExpectedBackupIntegrityKeySha256,'-BackupReceiptPrivateKeyPath',(Q $BackupReceiptPrivateKeyPath),'-ExpectedBackupReceiptPrivateKeySha256',$ExpectedBackupReceiptPrivateKeySha256,
    '-ExpectedBackupTargetFingerprint',$ExpectedBackupTargetFingerprint,'-ExpectedBackupContractSha256',(BackupContractSha256),'-ExpectedPgPassSha256',$ExpectedPgPassSha256,'-ExpectedNasIdentityHelperSha256',$ExpectedNasIdentityHelperSha256,'-SignerAccount',(Q $SignerAccount),
    '-ScheduledAuthorizationPath',(Q $ScheduledAuthorizationPath),'-ExpectedScheduledAuthorizationSha256',$ExpectedScheduledAuthorizationSha256,'-ScheduleAuthorizationPublicKeyPath',(Q $ScheduleAuthorizationPublicKeyPath),'-ExpectedScheduleAuthorizationPublicKeySha256',$ExpectedScheduleAuthorizationPublicKeySha256,
    '-AttestationVerifierPath',(Q $AttestationVerifierPath),'-ExpectedAttestationVerifierSha256',$ExpectedAttestationVerifierSha256,'-SignerScratchRoot',(Q $SignerScratchRoot),'-SignerReplayLedgerRoot',(Q $SignerReplayLedgerRoot)
  ) -join ' '
}
...
function New-BackupScheduleApprovalPlan([string]$IntendedAction){
  ...
  if($TaskName-notmatch'^[A-Za-z0-9][A-Za-z0-9 ._-]{2,127}$'-or$ReceiptTaskName-notmatch'^[A-Za-z0-9][A-Za-z0-9 ._-]{2,127}$'-or$ReceiptTaskName-ceq$TaskName){throw 'SCHEDULE_PLAN_TASK_NAME_INVALID'}
  $parameters=[ordered]@{taskName=$TaskName;receiptTaskName=$ReceiptTaskName}
  if($IntendedAction-ne'Remove'){
    ...
    $parameters.coreServiceAccount=$CoreServiceAccount;$parameters.edgeServiceAccount=$EdgeServiceAccount;$parameters.backupAccount=$BackupAccount;$parameters.signerAccount=$SignerAccount;$parameters.publisherBrokerPath=Get-ApprovalPath $PublisherBrokerPath 'SCHEDULE_PLAN_PUBLISHER_BROKER_REQUIRED';$parameters.publisherBrokerSha256=Get-ApprovalHash $ExpectedPublisherBrokerSha256 'SCHEDULE_PLAN_PUBLISHER_BROKER_HASH_REQUIRED';$parameters.backupReceiptPrivateKeyPath=Get-ApprovalPath $BackupReceiptPrivateKeyPath 'SCHEDULE_PLAN_RECEIPT_PRIVATE_KEY_REQUIRED';$parameters.signerScratchRoot=Get-ApprovalPath $SignerScratchRoot 'SCHEDULE_PLAN_SIGNER_SCRATCH_REQUIRED';$parameters.signerReplayLedgerRoot=Get-ApprovalPath $SignerReplayLedgerRoot 'SCHEDULE_PLAN_SIGNER_REPLAY_REQUIRED';$parameters.filesystemEvidencePath=Get-ApprovalPath $FileSystemEvidencePath 'SCHEDULE_PLAN_FILESYSTEM_PATH_REQUIRED';$parameters.filesystemEvidenceSha256=Get-ApprovalHash $ExpectedFileSystemEvidenceSha256 'SCHEDULE_PLAN_FILESYSTEM_HASH_REQUIRED';$parameters.principalRightsEvidencePath=Get-ApprovalPath $PrincipalRightsEvidencePath 'SCHEDULE_PLAN_RIGHTS_PATH_REQUIRED';$parameters.principalRightsScriptPath=Get-ApprovalPath $PrincipalRightsScriptPath 'SCHEDULE_PLAN_RIGHTS_SCRIPT_PATH_REQUIRED';$parameters.principalRightsScriptSha256=Get-ApprovalHash $ExpectedPrincipalRightsScriptSha256 'SCHEDULE_PLAN_RIGHTS_SCRIPT_HASH_REQUIRED';$parameters.maximumDatabaseDumpBytes=$MaximumDatabaseDumpBytes;$parameters.maximumBackupDurationSeconds=$MaximumBackupDurationSeconds;$parameters.backupContractSha256=BackupContractSha256;$parameters.recurringInvocationSha256=Digest (ExpectedArguments);$parameters.publisherInvocationSha256=Digest (ExpectedPublisherArguments);$parameters.publisherIntervalMinutes=15
  ...
  $target=if($IntendedAction-eq'Remove'){"Exact Windows scheduled tasks $TaskName and $ReceiptTaskName"}else{"Exact daily backup task plus 15-minute independent receipt publisher task"}
  $impact=if($IntendedAction-eq'Apply'){'Registers the backup task under the backup account and the receipt publisher task under the distinct signer account'}elseif($IntendedAction-eq'Remove'){'Unregisters only the exact two tasks; backup payload is retained'}else{'Verifies both exact enabled triggers and writes only the exact evidence output'}
  $rollback=if($IntendedAction-eq'Apply'){'Use a separately approved Remove plan for both exact tasks'}elseif($IntendedAction-eq'Remove'){'Recreate both only through a new exact Apply plan'}else{'Delete only the exact evidence output'}
...
$receiptTask=Get-ScheduledTask -TaskName $ReceiptTaskName -ErrorAction SilentlyContinue
if($Action -eq 'Verify') {
  foreach($file in @($PowerShellPath,$BackupScriptPath,$PublisherBrokerPath,$NodePath,$PsqlPath,$PgDumpPath,$PgRestorePath,$PgPassFile,$BackupIntegrityKeyFile,$BackupReceiptPrivateKeyPath,$ReceiptPublisherPath,$SemanticSignerPath,$NasIdentityHelperPath,$ReleaseVerifierPath,$AttestationVerifierPath,$ScheduledAuthorizationPath,$ScheduleAuthorizationPublicKeyPath,$FileSystemEvidencePath,$RuntimeConfigPath,$PrincipalRightsScriptPath,$PrincipalRightsEvidencePath)){if(-not(Test-Path -LiteralPath $file -PathType Leaf)){throw 'SCHEDULE_REQUIRED_FILE_NOT_FOUND'};NoReparse $file}
  Assert-Pinned $RuntimeConfigPath $ExpectedRuntimeConfigSha256 'RUNTIME_CONFIG_HASH_MISMATCH'
  Assert-Pinned $NodePath $ExpectedNodeSha256 'NODE_HASH_MISMATCH';Assert-Pinned $PsqlPath $ExpectedPsqlSha256 'PSQL_HASH_MISMATCH';Assert-Pinned $PgDumpPath $ExpectedPgDumpSha256 'PG_DUMP_HASH_MISMATCH';Assert-Pinned $PgRestorePath $ExpectedPgRestoreSha256 'PG_RESTORE_HASH_MISMATCH';Assert-Pinned $PgPassFile $ExpectedPgPassSha256 'PGPASS_HASH_MISMATCH';Assert-Pinned $BackupIntegrityKeyFile $ExpectedBackupIntegrityKeySha256 'BACKUP_INTEGRITY_KEY_HASH_MISMATCH';Assert-Pinned $BackupReceiptPrivateKeyPath $ExpectedBackupReceiptPrivateKeySha256 'BACKUP_RECEIPT_PRIVATE_KEY_HASH_MISMATCH';Assert-Pinned $ReceiptPublisherPath $ExpectedReceiptPublisherSha256 'RECEIPT_PUBLISHER_HASH_MISMATCH';Assert-Pinned $PublisherBrokerPath $ExpectedPublisherBrokerSha256 'PUBLISHER_BROKER_HASH_MISMATCH';Assert-Pinned $SemanticSignerPath $ExpectedSemanticSignerSha256 'SEMANTIC_SIGNER_HASH_MISMATCH';Assert-Pinned $NasIdentityHelperPath $ExpectedNasIdentityHelperSha256 'NAS_IDENTITY_HELPER_HASH_MISMATCH'
  ...
  $fs=Get-Content -Raw -LiteralPath $FileSystemEvidencePath|ConvertFrom-Json;$scheduleExecutors=@($NodePath,$PsqlPath,$PgDumpPath,$PgRestorePath,$ReceiptPublisherPath,$PublisherBrokerPath,$SemanticSignerPath,$NasIdentityHelperPath,$ReleaseVerifierPath,$AttestationVerifierPath);if($fs.result-ne'PASS'-or-not$fs.exactAcl-or@($scheduleExecutors|Where-Object{-not(Under $_ $fs.classRoots.SHARED_RUNTIME)}).Count-or-not(Under $ScheduledAuthorizationPath $fs.classRoots.ADMIN_EVIDENCE)-or-not(Under $RuntimeConfigPath $fs.classRoots.SHARED_RUNTIME)-or-not(Under $BackupReceiptPrivateKeyPath $fs.classRoots.SIGNER_ONLY)-or-not(Under $SignerScratchRoot $fs.classRoots.SIGNER_STATE)-or-not(Under $SignerReplayLedgerRoot $fs.classRoots.SIGNER_STATE)){throw 'SCHEDULE_EXECUTOR_OUTSIDE_SHARED_RUNTIME'}
  ...
  if(-not $task-or-not$receiptTask){ [pscustomobject]@{Result='FAIL';Reason='MISSING'}|ConvertTo-Json; exit 1 }
  ...
  $receiptTaskInfo=Get-ScheduledTaskInfo -TaskName $ReceiptTaskName
  ...
  if($rights.version-ne4-or$rights.result-ne'PASS'-or-not$rights.exactRights-or$rights.backupPrincipalDigest-cne(Digest $backupSid)-or$rights.signerPrincipalDigest-cne(Digest $signerSid)){throw 'PRINCIPAL_RIGHTS_EVIDENCE_REJECTED'}
  ...
  $pass=@($task.Actions).Count -eq 1 -and $task.Actions[0].Execute -ieq [IO.Path]::GetFullPath($PowerShellPath) -and
    ...
    (Get-FileHash -LiteralPath $PgRestorePath -Algorithm SHA256).Hash -ieq $ExpectedPgRestoreSha256 -and
    ...
    (Get-FileHash -LiteralPath $NasIdentityHelperPath -Algorithm SHA256).Hash -ieq $ExpectedNasIdentityHelperSha256 -and
    @($receiptTask.Actions).Count -eq 1 -and $receiptTask.Actions[0].Execute -ieq [IO.Path]::GetFullPath($PowerShellPath) -and $receiptTask.Actions[0].Arguments -ceq (ExpectedPublisherArguments) -and
    $receiptTaskSid -eq $signerSid -and $receiptTaskSid -ne $backupSid -and $receiptTask.State -ne 'Disabled' -and $receiptTask.Principal.RunLevel -eq 'Limited' -and @($receiptTask.Triggers).Count -eq 1 -and
    $receiptTriggerClass -ceq 'MSFT_TaskDailyTrigger' -and $receiptTask.Triggers[0].Enabled -eq $true -and [int]$receiptTask.Triggers[0].DaysInterval -eq 1 -and
    [string]$receiptTask.Triggers[0].Repetition.Interval -eq 'PT15M' -and [string]$receiptTask.Triggers[0].Repetition.Duration -eq 'P1D' -and
    $receiptTask.Settings.StartWhenAvailable -and $receiptTask.Settings.MultipleInstances -eq 'IgnoreNew' -and $receiptTask.Settings.ExecutionTimeLimit -eq 'PT4H' -and
    (Get-FileHash -LiteralPath $PublisherBrokerPath -Algorithm SHA256).Hash -ieq $ExpectedPublisherBrokerSha256 -and (Get-FileHash -LiteralPath $BackupReceiptPrivateKeyPath -Algorithm SHA256).Hash -ieq $ExpectedBackupReceiptPrivateKeySha256
  if($pass -and $EvidenceOutputPath){
    ...
    $evidence=[ordered]@{
      version=8;result='PASS';taskName=$TaskName;receiptTaskName=$ReceiptTaskName;dailyTime=$config.backup.dailyTime;dailyTriggerVerified=$true;dailyTriggerEnabled=$true;daysInterval=1
      receiptPublisherRecurringVerified=$true;receiptPublisherIntervalMinutes=15;runtimeConfigSha256=$ExpectedRuntimeConfigSha256.ToLowerInvariant();backupTargetEvidenceSha256=$ExpectedBackupTargetEvidenceSha256.ToLowerInvariant();backupTargetFingerprint=$ExpectedBackupTargetFingerprint.ToLowerInvariant();backupRoot=[IO.Path]::GetFullPath($ExpectedBackupRoot)
      scheduledAuthorizationSha256=$ExpectedScheduledAuthorizationSha256.ToLowerInvariant();scheduledAuthorizationVersion=4;scheduledAuthorizationBound=$true;runtimeReadinessBound=$true;recurringInvocationPlanBound=$true;maximumDatabaseDumpBytes=$MaximumDatabaseDumpBytes;maximumBackupDurationSeconds=$MaximumBackupDurationSeconds;backupSafetyMarginBytes=[long]1073741824
      databaseSizePreflightRequired=$true;databaseDumpRealtimeCapRequired=$true;databaseDumpFinalCapRequired=$true;hardDeadlineRequired=$true;processTreeKillOnDeadlineRequired=$true;incompleteStagingCleanupRequired=$true;backupSid=$backupSid;signerSid=$signerSid;separatePrincipal=$true;receiptSigningDelegatedToDistinctSigner=$true
      powerShell7Verified=$true;powerShellPath=[IO.Path]::GetFullPath($PowerShellPath);powerShellSha256=$ExpectedPowerShellSha256.ToLowerInvariant();actionArgumentsSha256=(Digest (ExpectedArguments));receiptPublisherActionArgumentsSha256=(Digest (ExpectedPublisherArguments));backupScriptSha256=$ExpectedBackupScriptSha256.ToLowerInvariant();publisherBrokerSha256=$ExpectedPublisherBrokerSha256.ToLowerInvariant()
      releaseVerifierSha256=$ExpectedReleaseVerifierSha256.ToLowerInvariant();releaseManifestSha256=$ExpectedReleaseManifestSha256.ToLowerInvariant();attestationVerifierSha256=$ExpectedAttestationVerifierSha256.ToLowerInvariant();nodeSha256=$ExpectedNodeSha256.ToLowerInvariant();psqlSha256=$ExpectedPsqlSha256.ToLowerInvariant();pgDumpSha256=$ExpectedPgDumpSha256.ToLowerInvariant();pgRestoreSha256=$ExpectedPgRestoreSha256.ToLowerInvariant();pgPassSha256=$ExpectedPgPassSha256.ToLowerInvariant();backupIntegrityKeySha256=$ExpectedBackupIntegrityKeySha256.ToLowerInvariant();backupReceiptPrivateKeySha256=$ExpectedBackupReceiptPrivateKeySha256.ToLowerInvariant();receiptPublisherSha256=$ExpectedReceiptPublisherSha256.ToLowerInvariant();semanticSignerSha256=$ExpectedSemanticSignerSha256.ToLowerInvariant();nasIdentityHelperPath=[IO.Path]::GetFullPath($NasIdentityHelperPath);nasIdentityHelperSha256=$ExpectedNasIdentityHelperSha256.ToLowerInvariant();executorSetDigest=$executorSetDigest;filesystemEvidenceSha256=$ExpectedFileSystemEvidenceSha256.ToLowerInvariant();scriptHashVerified=$true;signerHashVerified=$true;executorHashesVerified=$true;boundedChildProcesses=$true;enabled=$true;startWhenAvailable=$true
      lastObservedResult=$taskInfo.LastTaskResult;lastObservedRunAt=if($taskInfo.LastRunTime.Year-gt2000){$taskInfo.LastRunTime.ToUniversalTime().ToString('o')}else{$null};receiptTaskLastObservedResult=$receiptTaskInfo.LastTaskResult;receiptTaskLastObservedRunAt=if($receiptTaskInfo.LastRunTime.Year-gt2000){$receiptTaskInfo.LastRunTime.ToUniversalTime().ToString('o')}else{$null};completedAt=(Get-Date).ToUniversalTime().ToString('o')
    }
    [IO.File]::WriteAllText($EvidenceOutputPath,($evidence|ConvertTo-Json),(New-Object Text.UTF8Encoding($false)))
  }
  [pscustomobject]@{Result=if($pass){'PASS'}else{'FAIL'};LastTaskResult=$taskInfo.LastTaskResult;ReceiptTaskLastResult=$receiptTaskInfo.LastTaskResult;NextRunTime=$taskInfo.NextRunTime;ReceiptTaskNextRunTime=$receiptTaskInfo.NextRunTime}|ConvertTo-Json
  if(-not $pass){exit 1}; exit 0
}
if($Action -eq 'Remove'){
  if($task){Unregister-ScheduledTask -TaskName $TaskName -Confirm:$false}
  if($receiptTask){Unregister-ScheduledTask -TaskName $ReceiptTaskName -Confirm:$false}
  [pscustomobject]@{Result='REMOVED';BackupsRemoved=$false}|ConvertTo-Json; exit 0
}
if($task-or$receiptTask){throw 'BACKUP_TASK_ALREADY_EXISTS'}
Assert-PowerShell7 $PowerShellPath $ExpectedPowerShellSha256
Assert-Pinned $RuntimeConfigPath $ExpectedRuntimeConfigSha256 'RUNTIME_CONFIG_HASH_MISMATCH'
foreach($file in @($PowerShellPath,$BackupScriptPath,$PublisherBrokerPath,$RuntimeConfigPath,$PgDumpPath,$PsqlPath,$PgRestorePath,$PgPassFile,$BackupIntegrityKeyFile,$BackupReceiptPrivateKeyPath,$ReceiptPublisherPath,$SemanticSignerPath,$NasIdentityHelperPath,$NodePath,$ReleaseVerifierPath,$AttestationVerifierPath,$ScheduledAuthorizationPath,$ScheduleAuthorizationPublicKeyPath,$FileSystemEvidencePath,$PrincipalRightsEvidencePath,$PrincipalRightsScriptPath)) {
...
if(-not $SignerCredential -or -not $SignerAccount -or $SignerCredential.UserName -ine $SignerAccount -or $SignerAccount -ieq $BackupAccount){throw 'SEPARATE_SIGNER_CREDENTIAL_REQUIRED'}
...
if($SignerAccount-notmatch'^([^\\]+)\\([^\\]+)$'-or$Matches[1]-ine$env:COMPUTERNAME){throw 'LOCAL_SIGNER_ACCOUNT_REQUIRED'}
$localSigner=Get-LocalUser -Name $Matches[2] -ErrorAction Stop;if(-not$localSigner.Enabled){throw 'LOCAL_SIGNER_ACCOUNT_DISABLED'}
...
if($fs.result-ne'PASS'-or$fs.coreServiceSid-ne$coreSid-or$fs.edgeServiceSid-ne$edgeSid-or$fs.backupSid-ne$backupSid-or$fs.signerSid-cne$signerSid-or$fs.signerSid-cne$backupEvidence.signerReaderSid){throw 'FILESYSTEM_EVIDENCE_PRINCIPAL_MISMATCH'}
$rights=Get-Content -Raw -LiteralPath $PrincipalRightsEvidencePath|ConvertFrom-Json
if($rights.version-ne4-or$rights.result-ne'PASS'-or-not$rights.exactRights-or$rights.corePrincipalDigest-cne(Digest $coreSid)-or$rights.edgePrincipalDigest-cne(Digest $edgeSid)-or$rights.backupPrincipalDigest-cne(Digest $backupSid)-or$rights.signerPrincipalDigest-cne(Digest $signerSid)){throw 'PRINCIPAL_RIGHTS_EVIDENCE_REJECTED'}
if($ExpectedPrincipalRightsScriptSha256-notmatch'^[A-Fa-f0-9]{64}$'-or(Get-FileHash -LiteralPath $PrincipalRightsScriptPath -Algorithm SHA256).Hash-ine$ExpectedPrincipalRightsScriptSha256){throw 'PRINCIPAL_RIGHTS_SCRIPT_HASH_MISMATCH'}
Assert-Pinned $PrincipalRightsScriptPath $ExpectedPrincipalRightsScriptSha256 'PRINCIPAL_RIGHTS_SCRIPT_HASH_MISMATCH';Invoke-ScheduleBounded $PowerShellPath $ExpectedPowerShellSha256 @('-NoProfile','-NonInteractive','-File',$PrincipalRightsScriptPath,'-Action','Verify','-CoreServiceAccount',$CoreServiceAccount,'-EdgeServiceAccount',$EdgeServiceAccount,'-BackupAccount',$BackupAccount,'-SignerAccount',$SignerAccount) 'PRINCIPAL_RIGHTS_VERIFY_FAILED'|Out-Null
...
if(-not(Under $BackupReceiptPrivateKeyPath @($fs.classRoots.SIGNER_ONLY))-or-not(Under $SignerScratchRoot @($fs.classRoots.SIGNER_STATE))-or-not(Under $SignerReplayLedgerRoot @($fs.classRoots.SIGNER_STATE))){throw 'SIGNER_STATE_CLASS_REJECTED'}
...
$receiptAction=New-ScheduledTaskAction -Execute ([IO.Path]::GetFullPath($PowerShellPath)) -Argument (ExpectedPublisherArguments)
$receiptTrigger=New-ScheduledTaskTrigger -Daily -At ([datetime]::Today);$receiptTrigger.Repetition.Interval='PT15M';$receiptTrigger.Repetition.Duration='P1D';$receiptTrigger.Repetition.StopAtDurationEnd=$false
$receiptSettings=New-ScheduledTaskSettingsSet -StartWhenAvailable -ExecutionTimeLimit (New-TimeSpan -Hours 4) -MultipleInstances IgnoreNew -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries
$password=$BackupCredential.GetNetworkCredential().Password;$signerPassword=$SignerCredential.GetNetworkCredential().Password;$backupRegistered=$false
try {
  ...
  Register-ScheduledTask -TaskName $ReceiptTaskName -Action $receiptAction -Trigger $receiptTrigger -Settings $receiptSettings -User $SignerCredential.UserName -Password $signerPassword -RunLevel Limited | Out-Null
} catch {if($backupRegistered){Unregister-ScheduledTask -TaskName $TaskName -Confirm:$false -ErrorAction SilentlyContinue};throw} finally { $password=$null;$signerPassword=$null }
[pscustomobject]@{Result='APPLIED';RPOHours=24;PrincipalSeparated=$true;ReceiptPublisherRecurring=$true;ReceiptPublisherIntervalMinutes=15;FirstSuccessfulSignedBackupRequired=$true}|ConvertTo-Json
