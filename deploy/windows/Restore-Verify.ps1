...
param(
  ...
  [string]$RestoreReceiptPrivateKeyPath,
  ...
  [string]$PreviousApiConfigPath,[string]$TargetApiConfigPath,[string]$InternalProbeTokenFile,
  [ValidateRange(1024,65535)][int]$CompatibilityApiPort=42991,
  [string]$CompatibilityEvidenceOutputPath,
  [ValidateSet('Run')][string]$PlannedAction,[string]$ApprovedPlanSha256,[switch]$Approved
)
...
function New-RestoreApprovalPlan([string]$IntendedAction) {
  ...
  $p=[ordered]@{
    ...
    pgPassFile=Get-ApprovalPath $PgPassFile 'RESTORE_PLAN_PGPASS_PATH_REQUIRED';caCertificatePath=Get-ApprovalPath $RestoreCaCertificatePath 'RESTORE_PLAN_CA_PATH_REQUIRED';caCertificateSha256=Get-ApprovalHash $ExpectedRestoreCaCertificateSha256 'RESTORE_PLAN_CA_HASH_REQUIRED'
    integrityKeyFile=Get-ApprovalPath $BackupIntegrityKeyFile 'RESTORE_PLAN_INTEGRITY_KEY_PATH_REQUIRED';migrationsRoot=Get-ApprovalPath $MigrationsRoot 'RESTORE_PLAN_MIGRATIONS_ROOT_REQUIRED'
    ...
    signerPath=Get-ApprovalPath $AttestationSignerPath 'RESTORE_PLAN_SIGNER_PATH_REQUIRED';signerSha256=Get-ApprovalHash $ExpectedAttestationSignerSha256 'RESTORE_PLAN_SIGNER_HASH_REQUIRED';receiptPrivateKeyPath=Get-ApprovalPath $RestoreReceiptPrivateKeyPath 'RESTORE_PLAN_PRIVATE_KEY_PATH_REQUIRED'
...
$sourceTopLevel=@(Get-DeadlineBoundTreeItems -Path $sourceBackupRoot -TopLevelOnly|ForEach-Object{$_.Name}|Sort-Object);$sourceHasConversion=Test-Path -LiteralPath (Join-Path $sourceBackupRoot 'storage-reference-conversion.sql') -PathType Leaf;$expectedSourceTop=@('backup-manifest.json','database.dump','storage-manifest.json','storage-payload');if($sourceHasConversion){$expectedSourceTop+='storage-reference-conversion.sql'}
...
"@
  ...
  if($PreviousReleaseKind-eq'LOCAL_RELEASE'){$previousSmokeBeforeDigest=Invoke-ApiSmoke ([IO.Path]::GetFullPath($PreviousReleaseRoot)) ([string]$previousRelease.releaseId) $PreviousApiConfigPath $probeToken 'previous-before';$previousBusinessBeforeDigest=Invoke-BusinessSmoke ([IO.Path]::GetFullPath($PreviousReleaseRoot)) ([string]$previousRelease.releaseId) $PreviousApiConfigPath $smokeFrom $smokeTo;$previousRoleMatrixBeforeDigest=Invoke-RoleMatrixSmoke ([IO.Path]::GetFullPath($PreviousReleaseRoot)) ([string]$previousRelease.releaseId)}else{$previousSmokeBeforeDigest=Sha256Text 'ACTUAL_LEGACY_CODE_NOT_EXECUTED_IN_ISOLATED_RESTORE';$previousRoleMatrixBeforeDigest=Sha256Text 'LEGACY_LOCAL_ROLE_MATRIX_NOT_APPLICABLE';$previousBusinessBeforeDigest=Invoke-TargetLegacyDataProjectionSmoke ([IO.Path]::GetFullPath($TargetReleaseRoot)) ([string]$targetRelease.releaseId) $TargetApiConfigPath $smokeFrom $smokeTo}
...
$evidence = [ordered]@{
  ...
  rpoHours=24; rtoHours=4; elapsedSeconds=$elapsed; databaseRestored=$true; storageHashVerified=$true; storageReferenceVerified=$true; businessKpiVerified=$true
  ...
  migrationDigest=$manifest.migrationDigest; appliedMigrationDigest=$manifest.appliedMigrationDigest; businessKpiDigest=$manifest.businessKpiDigest; storageReferenceDigest=$manifest.storageReferenceDigest
...
$rehearsal=[ordered]@{
  ...
  previousSmokeBeforeDigest=$previousSmokeBeforeDigest;previousSmokeAfterDigest=$previousSmokeAfterDigest;targetSmokeAfterDigest=$targetSmokeAfterDigest;previousBusinessBeforeDigest=$previousBusinessBeforeDigest;previousBusinessAfterDigest=$previousBusinessAfterDigest;targetBusinessAfterDigest=$targetBusinessAfterDigest;previousRoleMatrixBeforeDigest=$previousRoleMatrixBeforeDigest;previousRoleMatrixAfterDigest=$previousRoleMatrixAfterDigest;targetRoleMatrixAfterDigest=$targetRoleMatrixAfterDigest;targetAppliedMigrationDigest=(Sha256Text $targetAppliedCanonical)
  authRoleMatrixVerified=$true;businessKpiVerified=$true;businessKpiDigest=$manifest.businessKpiDigest;storageHashVerified=$true;storageReferenceVerified=$true;storageReferenceDigest=$manifest.storageReferenceDigest
