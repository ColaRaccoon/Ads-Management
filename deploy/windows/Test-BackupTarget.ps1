...
param(
  ...
  [string]$CoreServiceAccount,[string]$EdgeServiceAccount,[string]$BackupWriterAccount,
  ...
  [ValidateSet('VerifyEvidence')][string]$PlannedAction,[string]$ApprovedPlanSha256,[switch]$Approved
)
...
function Assert-ExactBackupAcl([string]$Path,[string]$CoreSid,[string]$EdgeSid,[string]$WriterSid){
  ...
  $expected=@($systemSid,$administratorsSid,$WriterSid)|Sort-Object -Unique
  if(@($acl.Access).Count-ne3){throw 'BACKUP_TARGET_ACL_RULE_COUNT_INVALID'}
  $full=[Security.AccessControl.FileSystemRights]::FullControl;$modify=[Security.AccessControl.FileSystemRights]'Modify,Synchronize'
  foreach($rule in $acl.Access){
    $sid=$rule.IdentityReference.Translate([Security.Principal.SecurityIdentifier]).Value
    $rights=if($sid-in@($systemSid,$administratorsSid)){$full}elseif($sid-eq$WriterSid){$modify}else{$null}
...
function New-BackupTargetApprovalPlan([string]$IntendedAction){if($IntendedAction-cne'VerifyEvidence'){throw'BACKUP_TARGET_PLAN_ACTION_REQUIRED'};$p=[ordered]@{dataRoot=Get-ApprovalPath $DataRoot 'BACKUP_TARGET_PLAN_DATA_ROOT_REQUIRED';backupRoot=Get-ApprovalPath $BackupRoot 'BACKUP_TARGET_PLAN_BACKUP_ROOT_REQUIRED';evidencePath=Get-ApprovalPath $EvidencePath 'BACKUP_TARGET_PLAN_EVIDENCE_PATH_REQUIRED';coreServiceAccount=Get-ApprovalText $CoreServiceAccount '^.{1,256}$' 'BACKUP_TARGET_PLAN_CORE_ACCOUNT_REQUIRED';edgeServiceAccount=Get-ApprovalText $EdgeServiceAccount '^.{1,256}$' 'BACKUP_TARGET_PLAN_EDGE_ACCOUNT_REQUIRED';backupWriterAccount=Get-ApprovalText $BackupWriterAccount '^.{1,256}$' 'BACKUP_TARGET_PLAN_WRITER_ACCOUNT_REQUIRED';nasAdministrativelyConfirmed=[bool]$NasAdministrativelyConfirmed;nasShareAclAdministrativelyConfirmed=[bool]$NasShareAclAdministrativelyConfirmed;nasSnapshotOrVersioningConfirmed=[bool]$NasSnapshotOrVersioningConfirmed;offlineRotationConfirmed=[bool]$OfflineRotationConfirmed};New-ApprovalPlan $PSCommandPath 'VerifyEvidence' $p 'Exact configured data root, separate disk/NAS backup root, accounts, and evidence output' 'Performs read-only disk/NAS and ACL checks, then writes one non-secret backup-target receipt' 'Delete only the exact evidence JSON'}
...
$coreSid=Resolve-Sid $CoreServiceAccount;$edgeSid=Resolve-Sid $EdgeServiceAccount;$writerSid=Resolve-Sid $BackupWriterAccount
if(@(@($coreSid,$edgeSid,$writerSid)|Sort-Object -Unique).Count-ne3){throw 'SEPARATE_BACKUP_PRINCIPALS_REQUIRED'}
...
$targetType='LOCAL_DISK';$backupDisk=$null;$canonicalBackupRoot=$BackupRoot;$encryptionProof=$null;$nasServer=$null;$nasShare=$null
...
Assert-ExactBackupAcl $canonicalBackupRoot $coreSid $edgeSid $writerSid
...
$evidence=[ordered]@{version=5;result='PASS';targetType=$targetType;dataRoot=$data;backupRoot=$canonicalBackupRoot;dataDiskNumber=$dataDisk.Number;dataDiskUniqueId=$dataDisk.UniqueId;backupDiskNumber=if($backupDisk){$backupDisk.Number}else{$null};backupDiskUniqueId=if($backupDisk){$backupDisk.UniqueId}else{$null};nasServer=$nasServer;nasShare=$nasShare;backupWriterSid=$writerSid;appServiceDenied=$true;edgeServiceDenied=$true;separateBackupWriter=$true;aclProtected=$true;exactAcl=$true;encryptedAtRestOrTransport=$true;encryptionProof=$encryptionProof;retentionControl=if($targetType-eq'NAS'){'SNAPSHOT_OR_VERSIONING'}else{'OFFLINE_ROTATION'};nasShareAclAdministrativelyConfirmed=($targetType-eq'NAS');completedAt=(Get-Date).ToUniversalTime().ToString('o')}
