...
param(
  ...
  [string]$MaintenanceFlagPath,[string]$ExpectedMaintenanceFlagSha256,[ValidateSet('ACTIVE_LOCAL_EDGE','LEGACY_QUIESCED_NO_EDGE')][string]$EdgeStateMode='ACTIVE_LOCAL_EDGE',[string]$DrainStatePath,[string]$ExpectedDrainStateSha256,
...
function Get-CurrentEdgeIdentity {
  ...
  $listeners=@(Get-NetTCPConnection -State Listen -LocalPort 443 -ErrorAction SilentlyContinue);if($listeners.Count){throw 'ROLLBACK_LEGACY_MODE_EDGE_LISTENER_PRESENT'}
  $service=Get-CimInstance Win32_Service -Filter "Name='$EdgeServiceName'" -ErrorAction SilentlyContinue;if($service-and($service.State-cne'Stopped'-or$service.StartMode-notin@('Manual','Disabled'))){throw 'ROLLBACK_LEGACY_MODE_EDGE_SERVICE_UNSAFE'}
...
function New-RollbackPlan([string]$IntendedAction) {
  ...
  if($IntendedAction-eq'Apply'){$preserved=Get-PreservedSchemaName}else{
    ...
    if($existingRollbackRecord.version -ne 4 -or $existingRollbackRecord.previousReleaseKind -cne $PreviousReleaseKind -or -not $allowedState -or $preserved -notmatch '^__metaads_pre_[0-9a-f]{16}$' -or ([string]$existingRollbackRecord.databaseSchema -and [string]$existingRollbackRecord.databaseSchema -cne $ConfirmDatabaseSchema)){throw 'ROLLBACK_EXISTING_JOURNAL_STATE_REJECTED'}
  }
  $parameters = [ordered]@{
    provider='supabase_postgres';previousReleaseKind=$PreviousReleaseKind;projectRef=Get-ApprovalText $ConfirmProjectRef '^[a-z]{20}$' 'ROLLBACK_PROJECT_REQUIRED';host=Get-ApprovalText $ConfirmDatabaseHost '^(?:db\.[a-z]{20}\.supabase\.co|[a-z0-9-]+\.pooler\.supabase\.com)$' 'ROLLBACK_HOST_REQUIRED';port=5432;databaseName=Get-ApprovalText $ConfirmDatabaseName '^[a-z][a-z0-9_]{0,62}$' 'ROLLBACK_DATABASE_REQUIRED';databaseSchema=Get-ApprovalText $ConfirmDatabaseSchema '^[a-z][a-z0-9_]{0,62}$' 'ROLLBACK_SCHEMA_REQUIRED';preservedSchema=$preserved;databaseUser=Get-ApprovalText $ConfirmDatabaseUser '^[a-z][a-z0-9_]{0,62}$' 'ROLLBACK_DATABASE_USER_REQUIRED'
    runtimeConfigPath=Get-ApprovalPath $RuntimeConfigPath 'ROLLBACK_RUNTIME_REQUIRED';runtimeConfigSha256=Get-ApprovalHash $ExpectedRuntimeConfigSha256 'ROLLBACK_RUNTIME_HASH_REQUIRED';migrationJournalPath=Get-ApprovalPath $MigrationJournalPath 'ROLLBACK_MIGRATION_JOURNAL_REQUIRED';migrationJournalSha256=Get-ApprovalHash $ExpectedMigrationJournalSha256 'ROLLBACK_MIGRATION_JOURNAL_HASH_REQUIRED';rollbackJournalPath=Get-ApprovalPath $RollbackJournalPath 'ROLLBACK_DURABLE_JOURNAL_REQUIRED'
    maintenanceFlagPath=Get-ApprovalPath $MaintenanceFlagPath 'ROLLBACK_MAINTENANCE_REQUIRED';maintenanceFlagSha256=Get-ApprovalHash $ExpectedMaintenanceFlagSha256 'ROLLBACK_MAINTENANCE_HASH_REQUIRED';edgeStateMode=$EdgeStateMode;quiescenceProofType=$edge.proofType;edgeServiceName=$EdgeServiceName
    ...
    nodePath=Get-ApprovalPath $NodePath 'ROLLBACK_NODE_REQUIRED';nodeSha256=Get-ApprovalHash $ExpectedNodeSha256 'ROLLBACK_NODE_HASH_REQUIRED';verifierPath=Get-ApprovalPath $AttestationVerifierPath 'ROLLBACK_VERIFIER_REQUIRED';verifierSha256=Get-ApprovalHash $ExpectedAttestationVerifierSha256 'ROLLBACK_VERIFIER_HASH_REQUIRED';recoveryProcessTreeHelperPath=Get-ApprovalPath $recoveryProcessTreeHelper 'ROLLBACK_PROCESS_TREE_HELPER_REQUIRED';recoveryProcessTreeHelperSha256=Get-ApprovalHash $ExpectedRecoveryProcessTreeHelperSha256 'ROLLBACK_PROCESS_TREE_HELPER_HASH_REQUIRED';psqlPath=Get-ApprovalPath $PsqlPath 'ROLLBACK_PSQL_REQUIRED';psqlSha256=Get-ApprovalHash $ExpectedPsqlSha256 'ROLLBACK_PSQL_HASH_REQUIRED';pgRestorePath=Get-ApprovalPath $PgRestorePath 'ROLLBACK_PGRESTORE_REQUIRED';pgRestoreSha256=Get-ApprovalHash $ExpectedPgRestoreSha256 'ROLLBACK_PGRESTORE_HASH_REQUIRED';pgPassPath=Get-ApprovalPath $PgPassFile 'ROLLBACK_PGPASS_REQUIRED';pgPassSha256=Get-ApprovalHash $ExpectedPgPassSha256 'ROLLBACK_PGPASS_HASH_REQUIRED';caPath=Get-ApprovalPath $CaCertificatePath 'ROLLBACK_CA_REQUIRED';caSha256=Get-ApprovalHash $ExpectedCaCertificateSha256 'ROLLBACK_CA_HASH_REQUIRED'
...
$hashInputs=@(
  @($runtime,$ExpectedRuntimeConfigSha256,'ROLLBACK_RUNTIME_HASH_MISMATCH',16777216),@($migrationJournal,$ExpectedMigrationJournalSha256,'ROLLBACK_MIGRATION_JOURNAL_HASH_MISMATCH',16777216),@($maintenancePath,$ExpectedMaintenanceFlagSha256,'ROLLBACK_MAINTENANCE_HASH_MISMATCH',1048576),@($backupEvidence,$ExpectedBackupEvidenceSha256,'ROLLBACK_BACKUP_EVIDENCE_HASH_MISMATCH',16777216),@($backupPublic,$ExpectedBackupReceiptPublicKeySha256,'ROLLBACK_BACKUP_KEY_HASH_MISMATCH',1048576),@($integrityKey,$ExpectedBackupIntegrityKeySha256,'ROLLBACK_INTEGRITY_KEY_HASH_MISMATCH',1024),@($node,$ExpectedNodeSha256,'ROLLBACK_NODE_HASH_MISMATCH',1073741824),@($verifier,$ExpectedAttestationVerifierSha256,'ROLLBACK_VERIFIER_HASH_MISMATCH',16777216),@($processTreeHelper,$ExpectedRecoveryProcessTreeHelperSha256,'ROLLBACK_PROCESS_TREE_HELPER_HASH_MISMATCH',16777216),@($psql,$ExpectedPsqlSha256,'ROLLBACK_PSQL_HASH_MISMATCH',1073741824),@($pgRestore,$ExpectedPgRestoreSha256,'ROLLBACK_PGRESTORE_HASH_MISMATCH',1073741824),@($pgpass,$ExpectedPgPassSha256,'ROLLBACK_PGPASS_HASH_MISMATCH',1048576),@($ca,$ExpectedCaCertificateSha256,'ROLLBACK_CA_HASH_MISMATCH',1048576),@($fsPath,$ExpectedFileSystemEvidenceSha256,'ROLLBACK_FS_HASH_MISMATCH',16777216)
)
...
if($PreviousReleaseKind-eq'LEGACY_BASELINE'){
  ...
  if($migration.previousReleaseId-cne$config.release.id-or$migration.quiescenceProofType-cne'SIGNED_EDGE_DRAIN_V2'-or$migration.drainEvidenceSha256-cne$ExpectedDrainStateSha256.ToLowerInvariant()-or$migration.legacyQuiesceEvidenceSha256){throw 'ROLLBACK_LOCAL_RELEASE_JOURNAL_REJECTED'}
...
if ($Action -eq 'Apply') {
  ...
  try {
    ...
    try {
      $unsignedEvidence = [ordered]@{attestationType='database-rollback';version=4;previousReleaseKind=$PreviousReleaseKind;result='PASS';rollbackIntentSha256=$intentSha256;rollbackPlanSha256=$approvalPlan.planSha256;recoveryProcessTreeHelperPath=$processTreeHelper;recoveryProcessTreeHelperSha256=$ExpectedRecoveryProcessTreeHelperSha256.ToLowerInvariant();baselineSha256=if($PreviousReleaseKind-eq'LEGACY_BASELINE'){(Hash $baselinePath 16777216)}else{$null};legacyQuiesceEvidenceSha256=$quiesceIdentity.EvidenceSha256;quiescePublicKeySha256=$quiesceIdentity.PublicKeySha256;quiesceSigningKeyId=$quiesceIdentity.SigningKeyId;quiesceCompletedAt=$quiesceIdentity.CompletedAt;legacyProcessIdentityDigest=$quiesceIdentity.ProcessIdentityDigest;legacyRestartCanonicalDigest=$quiesceIdentity.RestartCanonicalDigest;legacyWebProcessId=$quiesceIdentity.WebProcessId;legacyApiProcessId=$quiesceIdentity.ApiProcessId;migrationJournalSha256=(Hash $migrationJournal 16777216);migrationDigest=$previousMigrationDigest;appliedMigrationDigest=$previousAppliedMigrationDigest;releaseId=if($PreviousReleaseKind-eq'LOCAL_RELEASE'){$config.release.id}else{$baseline.releaseId};releaseManifestSha256=if($PreviousReleaseKind-eq'LOCAL_RELEASE'){$ExpectedEdgeReleaseManifestSha256.ToLowerInvariant()}else{$null};backupEvidenceSha256=(Hash $backupEvidence 16777216);backupId=$manifest.backupId;backupManifestSha256=(Hash $manifestPath 16777216);databaseProjectRef=$ConfirmProjectRef;databaseHost=$ConfirmDatabaseHost;databaseName=$ConfirmDatabaseName;databaseSchema=$ConfirmDatabaseSchema;preRollbackSchemaPreserved=$true;preservedSchema=$preservedSchema;productionDatabaseRestored=$true;runtimeRolePrivilegesVerified=$true;maintenanceVerified=$true;edgeStateMode=$EdgeStateMode;quiescenceProofType=$edgeIdentity.proofType;quiescenceVerified=$true;drainVerified=($EdgeStateMode-eq'ACTIVE_LOCAL_EDGE');legacyNoEdgeVerified=($EdgeStateMode-eq'LEGACY_QUIESCED_NO_EDGE');drainStateSha256=$edgeIdentity.drainStateSha256;drainCompletedAt=$edgeIdentity.drainCompletedAt;edgeIdentityDigest=$edgeIdentity.identityDigest;edgeProcessId=$edgeIdentity.processId;edgeProcessStartedAt=$edgeIdentity.processStartedAt;edgeExecutableSha256=$edgeIdentity.executableSha256;edgeReleaseId=$edgeIdentity.releaseId;edgeReleaseManifestSha256=$edgeIdentity.releaseManifestSha256;edgeListenerAddress=$edgeIdentity.listenerAddress;edgeListenerPort=$edgeIdentity.listenerPort;activeRequestsAtMutation=0;legacyWritersQuiesced=$true;businessKpiVerified=$true;businessKpiDigest=$manifest.businessKpiDigest;storageHashVerified=$true;storageInventoryDigest=$storageDigest;hardDeadlineEnforced=$true;processTreeKillOnDeadline=$true;allReadsAndHashesDeadlineBound=$true;elapsedSeconds=[int][Math]::Ceiling($script:RestoreWatch.Elapsed.TotalSeconds);maintenanceMustRemainEnabled=$true;recoveryProcedure="KEEP MAINTENANCE AND ALL WRITERS QUIESCED. RETURN FORWARD ONLY WITH A NEW EXACT APPROVED TRANSACTIONAL DROP-CURRENT AND RENAME-PRESERVED PLAN. RETRY $PreviousReleaseKind RESTORE ONLY WITH A NEW EXACT APPROVED RESTORE. TRANSITION ONLY AFTER SIGNED VERIFY.";completedAt=[datetimeoffset]::UtcNow.ToString('o')}
    ...
    $originalRestored = $false;$observedSchemaState = 'UNKNOWN';$preservedObserved = $false
    try {
    ...
    try {
      ...
      Write-AtomicJson $RollbackJournalPath ([ordered]@{version=4;previousReleaseKind=$PreviousReleaseKind;state='FAILED_MAINTENANCE_REQUIRED';intentSha256=$intentSha256;failureCode=$failureCode;edgeStateMode=$EdgeStateMode;quiescenceProofType=$edgeIdentity.proofType;edgeIdentityDigest=$edgeIdentity.identityDigest;recoveryProcessTreeHelperPath=$processTreeHelper;recoveryProcessTreeHelperSha256=$ExpectedRecoveryProcessTreeHelperSha256.ToLowerInvariant();maintenanceMustRemainEnabled=$true;legacyWritersMustRemainQuiesced=$true;legacyQuiesceEvidenceSha256=$quiesceIdentity.EvidenceSha256;databaseSchema=$ConfirmDatabaseSchema;preservedSchema=$preservedSchema;firstMutationAttempted=$boundaryMutationAttempted;boundaryCommandReportedSuccess=$boundaryCreated;observedSchemaState=$observedSchemaState;originalSchemaRestored=$originalRestored;originalSchemaPreserved=($preservedObserved -and -not $originalRestored);partialTargetPossible=($boundaryMutationAttempted -and -not $originalRestored);freshSchemaStateReconciled=($observedSchemaState-ne'UNKNOWN');recoveryProcedure=$recoveryProcedure;failedAt=[datetimeoffset]::UtcNow.ToString('o')})
...
$storageDigest = Get-RollbackStorageDigest
$edgeModeProof=if($EdgeStateMode-eq'ACTIVE_LOCAL_EDGE'){$value.quiescenceProofType-ceq'SIGNED_EDGE_DRAIN_V2'-and$value.drainVerified-and-not$value.legacyNoEdgeVerified}else{$value.quiescenceProofType-ceq'SIGNED_LEGACY_QUIESCE_V2'-and$value.legacyNoEdgeVerified-and-not$value.drainVerified}
