...
if ($Action -eq 'Apply') {
  ...
  try {
    ...
    if ($privilegeProof -cne 't|t|t|t|t|t|t') { throw 'ROLLBACK_RUNTIME_ROLE_PRIVILEGES_REJECTED' }
...
if ($journalValue.version -ne 3 -or $journalValue.state -cne 'COMPLETE_MAINTENANCE_REQUIRED' -or $journalValue.edgeStateMode-cne$EdgeStateMode -or $journalValue.quiescenceProofType-cne$value.quiescenceProofType -or $journalValue.edgeIdentityDigest-cne$edgeIdentity.identityDigest -or $journalValue.intentSha256 -cne $value.rollbackIntentSha256 -or $journalValue.evidenceSha256 -cne (Hash $evidence 16777216) -or $journalValue.recoveryProcessTreeHelperPath -ine $processTreeHelper -or $journalValue.recoveryProcessTreeHelperSha256 -cne $ExpectedRecoveryProcessTreeHelperSha256.ToLowerInvariant() -or $journalValue.legacyQuiesceEvidenceSha256 -cne $quiesceIdentity.EvidenceSha256 -or -not $journalValue.originalSchemaPreserved -or $journalValue.partialTargetPossible -or $value.version -ne 3 -or $value.result -cne 'PASS' -or $value.edgeStateMode-cne$EdgeStateMode -or $value.edgeIdentityDigest-cne$edgeIdentity.identityDigest -or -not$value.quiescenceVerified -or -not$edgeModeProof -or $value.recoveryProcessTreeHelperPath -ine $processTreeHelper -or $value.recoveryProcessTreeHelperSha256 -cne $ExpectedRecoveryProcessTreeHelperSha256.ToLowerInvariant() -or $value.baselineSha256 -cne (Hash $baselinePath 16777216) -or $value.legacyQuiesceEvidenceSha256 -cne $quiesceIdentity.EvidenceSha256 -or $value.quiescePublicKeySha256 -cne $quiesceIdentity.PublicKeySha256 -or $value.quiesceSigningKeyId -cne $quiesceIdentity.SigningKeyId -or $value.legacyProcessIdentityDigest -cne $quiesceIdentity.ProcessIdentityDigest -or $value.legacyRestartCanonicalDigest -cne $quiesceIdentity.RestartCanonicalDigest -or $value.migrationJournalSha256 -cne (Hash $migrationJournal 16777216) -or $value.backupEvidenceSha256 -cne (Hash $backupEvidence 16777216) -or -not $value.productionDatabaseRestored -or -not $value.preRollbackSchemaPreserved -or -not $value.runtimeRolePrivilegesVerified -or $privilegeProof -cne 't|t|t|t|t|t|t' -or -not $value.maintenanceVerified -or -not $value.businessKpiVerified -or (TextHash $kpi) -cne $value.businessKpiDigest -or -not $value.storageHashVerified -or $storageDigest -cne $value.storageInventoryDigest -or -not $value.hardDeadlineEnforced -or -not $value.processTreeKillOnDeadline -or -not $value.allReadsAndHashesDeadlineBound -or $value.elapsedSeconds -gt $MaximumRestoreDurationSeconds) { throw 'ROLLBACK_EVIDENCE_INVALID' }
Assert-RestoreDeadline -Finalization
