...
function Assert-DailyStoragePaths([string]$Live,[string]$Staging,[string]$Preserved) {
  $dataRoot=[IO.Path]::GetFullPath([string]$config.data.root).TrimEnd('\')
  $expectedLive=Join-Path $dataRoot 'storage'
  if([IO.Path]::GetFullPath($Live)-ine$expectedLive-or(Split-Path -Parent ([IO.Path]::GetFullPath($Staging)))-ine$dataRoot-or(Split-Path -Parent ([IO.Path]::GetFullPath($Preserved)))-ine$dataRoot-or(Split-Path -Leaf $Staging)-notmatch'^\.storage-recovery-[0-9a-f]{16}$'-or(Split-Path -Leaf $Preserved)-notmatch'^\.storage-preserved-[0-9a-f]{16}$'-or[IO.Path]::GetFullPath($Staging)-ieq[IO.Path]::GetFullPath($Preserved)){throw 'ROLLBACK_DAILY_STORAGE_PATH_REJECTED'}
  [void](Existing $dataRoot $true 'ROLLBACK_DATA_ROOT_NOT_FOUND')
}
function Copy-VerifiedStoragePayload([string]$Source,[string]$Target) {
  ...
  try{
    ...
    $sourceDigest=InventoryDigest $sourceRoot;$targetDigest=InventoryDigest $Target;if($sourceDigest-cne$targetDigest){throw 'ROLLBACK_STORAGE_STAGING_INVENTORY_MISMATCH'};return $targetDigest
  }catch{if(Test-Path -LiteralPath $Target){Remove-Item -LiteralPath $Target -Recurse -Force};throw}
}
function Restore-OriginalStorage([string]$Live,[string]$Staging,[string]$Preserved,[string]$ExpectedOriginalDigest) {
  Assert-DailyStoragePaths $Live $Staging $Preserved
...
function Get-CurrentEdgeIdentity {
  ...
  if($EdgeStateMode-eq'STOPPED_LOCAL_EDGE'){
    ...
    $identityDigest=TextHash (@('edge-daily-recovery-stopped-v1',$ExpectedRuntimeConfigSha256.ToLowerInvariant(),$releaseRoot,$ExpectedEdgeReleaseManifestSha256.ToLowerInvariant(),[string]$config.release.id,$node,$ExpectedNodeSha256.ToLowerInvariant(),$serviceState,'443-absent')-join"`n")
    return [pscustomobject]@{stateMode=$EdgeStateMode;processId=0;processStartedAt=$null;executablePath=$node;executableSha256=$ExpectedNodeSha256.ToLowerInvariant();commandLineSha256=$null;releaseRoot=$releaseRoot;releaseId=[string]$config.release.id;releaseManifestSha256=$ExpectedEdgeReleaseManifestSha256.ToLowerInvariant();listenerAddress=$null;listenerPort=0;serviceProcessId=0;activeRequests=0;drainCompletedAt=$null;drainStateSha256=$null;identityDigest=$identityDigest;proofType='STOPPED_LOCAL_EDGE_ABSENCE_V1';edgeServiceState=$serviceState}
  }
...
function New-RollbackPlan([string]$IntendedAction) {
  ...
  if($RecoveryPurpose-eq'MIGRATION_ROLLBACK'){
    ...
    if($IntendedAction-eq'Apply'){$runtimeForPlan=Read-BoundedJson (Existing $RuntimeConfigPath $false 'ROLLBACK_RUNTIME_NOT_FOUND') 16777216 'ROLLBACK_RUNTIME_INVALID';$dataRoot=[IO.Path]::GetFullPath([string]$runtimeForPlan.data.root);$suffix=$context.instanceId.Replace('-','').Substring(0,16);$parameters.liveStorageRoot=Get-ApprovalPath (Join-Path $dataRoot 'storage') 'ROLLBACK_LIVE_STORAGE_REQUIRED';$parameters.stagingStorageRoot=Get-ApprovalPath (Join-Path $dataRoot ('.storage-recovery-'+$suffix)) 'ROLLBACK_STAGING_STORAGE_REQUIRED';$parameters.preservedStorageRoot=Get-ApprovalPath (Join-Path $dataRoot ('.storage-preserved-'+$suffix)) 'ROLLBACK_PRESERVED_STORAGE_REQUIRED'}else{$parameters.liveStorageRoot=Get-ApprovalPath ([string]$existingRollbackRecord.liveStorageRoot) 'ROLLBACK_LIVE_STORAGE_REQUIRED';$parameters.stagingStorageRoot=Get-ApprovalPath ([string]$existingRollbackRecord.stagingStorageRoot) 'ROLLBACK_STAGING_STORAGE_REQUIRED';$parameters.preservedStorageRoot=Get-ApprovalPath ([string]$existingRollbackRecord.preservedStorageRoot) 'ROLLBACK_PRESERVED_STORAGE_REQUIRED'}
  }
...
$liveStorageRoot=$null;$stagingStorageRoot=$null;$preservedStorageRoot=$null;$backupPayloadRoot=$null
if($RecoveryPurpose-eq'DAILY_BACKUP_RECOVERY'){
  $liveStorageRoot=[IO.Path]::GetFullPath([string]$approvalPlan.exactParameters.liveStorageRoot);$stagingStorageRoot=[IO.Path]::GetFullPath([string]$approvalPlan.exactParameters.stagingStorageRoot);$preservedStorageRoot=[IO.Path]::GetFullPath([string]$approvalPlan.exactParameters.preservedStorageRoot);$backupPayloadRoot=Existing (Join-Path $backupRoot 'storage-payload') $true 'ROLLBACK_STORAGE_PAYLOAD_NOT_FOUND';Assert-DailyStoragePaths $liveStorageRoot $stagingStorageRoot $preservedStorageRoot
  if(-not(Under $liveStorageRoot $fs.classRoots.CORE_WRITE)-or-not(Under $stagingStorageRoot $fs.classRoots.CORE_WRITE)-or-not(Under $preservedStorageRoot $fs.classRoots.CORE_WRITE)){throw 'ROLLBACK_DAILY_STORAGE_ACL_BOUNDARY_REJECTED'}
}
...
if($Action-eq'Recover'){
  ...
  try{
    ...
    $storageOriginalRestored=$true;if($RecoveryPurpose-eq'DAILY_BACKUP_RECOVERY'){$storageOriginalRestored=Restore-OriginalStorage $liveStorageRoot $stagingStorageRoot $preservedStorageRoot ([string]$recoverySource.originalStorageInventoryDigest)}
    ...
    if($observed-ceq'1|0'){
      ...
      if($RecoveryPurpose-eq'DAILY_BACKUP_RECOVERY'){
        try{$storageOriginalRestored=Restore-OriginalStorage $liveStorageRoot $stagingStorageRoot $preservedStorageRoot ([string]$recoverySource.originalStorageInventoryDigest)}catch{$storageOriginalRestored=$false}
      }
...
if ($Action -eq 'Apply') {
  ...
  try {
    ...
    if($RecoveryPurpose-eq'DAILY_BACKUP_RECOVERY'){
      $stagedStorageDigest=Copy-VerifiedStoragePayload $backupPayloadRoot $stagingStorageRoot
    ...
    if($RecoveryPurpose-eq'DAILY_BACKUP_RECOVERY'){
      ...
      [IO.Directory]::Move($liveStorageRoot,$preservedStorageRoot);[IO.Directory]::Move($stagingStorageRoot,$liveStorageRoot)
    ...
    if($RecoveryPurpose-eq'DAILY_BACKUP_RECOVERY'-and$originalStorageDigest){try{$originalStorageRestored=Restore-OriginalStorage $liveStorageRoot $stagingStorageRoot $preservedStorageRoot $originalStorageDigest}catch{$originalStorageRestored=$false}}
