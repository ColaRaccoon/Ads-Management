...
if ($Action -eq 'Apply') {
  ...
  try {
    ...
    Write-AtomicJson $RollbackJournalPath ([ordered]@{version=2;state='COMPLETE_MAINTENANCE_REQUIRED';intentSha256=$intentSha256;evidenceSha256=$evidenceHash;maintenanceMustRemainEnabled=$true;legacyWritersMustRemainQuiesced=$true;legacyQuiesceEvidenceSha256=$quiesceIdentity.EvidenceSha256;databaseSchema=$ConfirmDatabaseSchema;preservedSchema=$preservedSchema;originalSchemaPreserved=$true;partialTargetPossible=$false;productionDatabaseRestored=$true;completedAt=[datetimeoffset]::UtcNow.ToString('o')})
    Assert-RestoreDeadline -Finalization
