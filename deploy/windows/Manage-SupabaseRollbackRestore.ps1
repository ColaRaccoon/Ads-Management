...
$helperCursor=if(Test-Path -LiteralPath $recoveryProcessTreeHelper -PathType Leaf){Get-Item -LiteralPath $recoveryProcessTreeHelper -Force}else{$null};while($helperCursor){if($helperCursor.Attributes-band[IO.FileAttributes]::ReparsePoint){throw 'ROLLBACK_PROCESS_TREE_HELPER_REPARSE_REJECTED'};$helperCursor=$helperCursor.Parent}
if(-not(Test-Path -LiteralPath $recoveryProcessTreeHelper -PathType Leaf)-or$ExpectedRecoveryProcessTreeHelperSha256-notmatch'^[A-Fa-f0-9]{64}$'-or(Get-FileHash -LiteralPath $recoveryProcessTreeHelper -Algorithm SHA256).Hash-ine$ExpectedRecoveryProcessTreeHelperSha256){throw 'ROLLBACK_PROCESS_TREE_HELPER_HASH_MISMATCH'}
. $recoveryProcessTreeHelper
...
if ($Action -eq 'Apply') {
  ...
  try {
    ...
    Write-AtomicJson $RollbackJournalPath ([ordered]@{version=2;state='COMPLETE_MAINTENANCE_REQUIRED';intentSha256=$intentSha256;evidenceSha256=$evidenceHash;recoveryProcessTreeHelperPath=$processTreeHelper;recoveryProcessTreeHelperSha256=$ExpectedRecoveryProcessTreeHelperSha256.ToLowerInvariant();maintenanceMustRemainEnabled=$true;legacyWritersMustRemainQuiesced=$true;legacyQuiesceEvidenceSha256=$quiesceIdentity.EvidenceSha256;databaseSchema=$ConfirmDatabaseSchema;preservedSchema=$preservedSchema;originalSchemaPreserved=$true;partialTargetPossible=$false;productionDatabaseRestored=$true;completedAt=[datetimeoffset]::UtcNow.ToString('o')})
    Assert-RestoreDeadline -Finalization
