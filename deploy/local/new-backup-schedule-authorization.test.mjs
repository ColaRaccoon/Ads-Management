...
test("daily backup authorization v4 exactly binds secret, signer, archive verifier, and NAS helper fingerprints", async () => {
  ...
  const values = ["1","2","3","4","5","6","7","8"].map((value) => value.repeat(64));
  ...
  assert.equal(authorization.version, 4);
  ...
  assert.equal(authorization.pgRestoreSha256, values[7]);
...
test("daily backup authorization refuses an omitted pinned key fingerprint", async () => {
  ...
  await writeFile(privatePath, pair.privateKey.export({ type:"pkcs8", format:"pem" }));
  const now = new Date();
  const args = [privatePath,"1".repeat(64),"Meta Ads Performance Daily Backup",now.toISOString(),new Date(now.valueOf()+86400_000).toISOString(),"a".repeat(64),"12345678-1234-4123-8123-123456789abc","2".repeat(64),"3".repeat(64),"4".repeat(64),"5".repeat(64),"6".repeat(64),"7".repeat(64),"",path.join(root,"authorization.json")];
