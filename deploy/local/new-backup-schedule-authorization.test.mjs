...
test("daily backup authorization v2 exactly binds secret and signer fingerprints", async () => {
  ...
  const values = ["1","2","3","4","5","6"].map((value) => value.repeat(64));
...
test("daily backup authorization refuses an omitted pinned key fingerprint", async () => {
  ...
  await writeFile(privatePath, pair.privateKey.export({ type:"pkcs8", format:"pem" }));
  const now = new Date();
  const args = [privatePath,"1".repeat(64),"Meta Ads Performance Daily Backup",now.toISOString(),new Date(now.valueOf()+86400_000).toISOString(),"a".repeat(64),"12345678-1234-4123-8123-123456789abc","2".repeat(64),"3".repeat(64),"","5".repeat(64),"6".repeat(64),path.join(root,"authorization.json")];
