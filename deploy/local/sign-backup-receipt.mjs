...
const requestKeys = ["attestationType","backupContractSha256","backupId","backupIntegrityKeySha256","backupReceiptPrivateKeySha256","backupRoot","completedAt","manifestSha256","nasIdentityHelperSha256","pgPassSha256","receiptPublisherSha256","result","semanticSignerSha256","targetEvidenceFingerprint","version"];
if (!exactObject(request, requestKeys) || request.attestationType !== "backup-latest" || request.version !== 6 || request.result !== "COMPLETE" || !backupId(request.backupId) || !hex(request.manifestSha256) || !hex(request.targetEvidenceFingerprint) || !hex(request.backupContractSha256) || !hex(request.pgPassSha256) || !hex(request.backupIntegrityKeySha256) || !hex(request.backupReceiptPrivateKeySha256) || !hex(request.receiptPublisherSha256) || !hex(request.semanticSignerSha256) || !hex(request.nasIdentityHelperSha256) || !timestamp(request.completedAt)) fail("BACKUP_RECEIPT_REQUEST_INVALID");
...
const artifactSnapshotBefore=artifactTreeDigest(artifactRoot);
...
function hashFile(file, maximum, code) { tick(); const before=lstatSync(file,{bigint:true}); if(!before.isFile()||before.isSymbolicLink()||before.nlink!==1n||before.size<0n||before.size>BigInt(maximum)) fail(code); const fd=openSync(file,"r"); const hash=createHash("sha256"); const buffer=Buffer.allocUnsafe(1_048_576); let bytes=0; try{const opened=fstatSync(fd,{bigint:true});assertSameFile(before,opened,code);for(;;){tick();const read=readSync(fd,buffer,0,buffer.length,null);if(!read)break;bytes+=read;if(bytes>maximum)fail(code);hash.update(buffer.subarray(0,read));}const after=fstatSync(fd,{bigint:true});assertSameFile(before,after,code)}finally{buffer.fill(0);closeSync(fd)} const finalPath=lstatSync(file,{bigint:true});assertSameFile(before,finalPath,code);if(BigInt(bytes)!==before.size)fail(code); return {bytes,sha256:hash.digest("hex")}; }
function bytesFile(file, maximum, code) { const result=hashFile(file,maximum,code);if(result.bytes<1)fail(code);const fd=openSync(file,"r"),bytes=Buffer.alloc(result.bytes);let offset=0;try{const before=fstatSync(fd,{bigint:true});if(before.nlink!==1n)fail(code);while(offset<bytes.length){tick();const count=readSync(fd,bytes,offset,bytes.length-offset,null);if(!count)break;offset+=count}const after=fstatSync(fd,{bigint:true});assertSameFile(before,after,code)}finally{closeSync(fd)}if(offset!==bytes.length||sha256(bytes)!==result.sha256){bytes.fill(0);fail(code)}return bytes; }
...
function artifactTreeDigest(root){const lines=[],pending=[root];let count=0,total=0;while(pending.length){tick();const directory=pending.pop();const stat=lstatSync(directory,{bigint:true});if(!stat.isDirectory()||stat.isSymbolicLink()||stat.nlink<1n)fail("BACKUP_ARTIFACT_TREE_INVALID");for(const item of readdirSync(directory,{withFileTypes:true})){const full=safeChild(directory,item.name),entry=lstatSync(full,{bigint:true});if(entry.isSymbolicLink())fail("BACKUP_REPARSE_REJECTED");if(entry.isDirectory()){pending.push(full);continue}if(!entry.isFile()||entry.nlink!==1n)fail("BACKUP_HARDLINK_REJECTED");const hashed=hashFile(full,274_877_906_944,"BACKUP_ARTIFACT_TREE_INVALID"),relative=path.relative(root,full).split(path.sep).join("/");count+=1;total+=hashed.bytes;if(count>1_000_010||total>1_100_000_000_000)fail("BACKUP_ARTIFACT_TREE_LIMIT");lines.push(`${relative}|${hashed.bytes}|${hashed.sha256}`)}}return sha256(Buffer.from(lines.sort().join("\n"),"utf8"));}
function assertSameFile(expected,actual,code){if(!actual.isFile()||actual.nlink!==1n||actual.dev!==expected.dev||actual.ino!==expected.ino||actual.size!==expected.size||actual.mtimeNs!==expected.mtimeNs)fail(code)}
