...
function validLatestBackupEvidence(evidence, publicKey, release, backup, dataRoot, database, targetEvidence, scheduleEvidence, now) {
  ...
  return Boolean(backup.latestBackupEvidencePath && evidence?.version === 6 && evidence?.result === "COMPLETE" &&
    evidence.attestationType === "backup-latest" && verifyAttestation(evidence, publicKey) && evidence.signerIndependentArtifactVerification === true && evidence.archiveTocVerified === true && /^[0-9a-f]{64}$/.test(evidence.archiveTocSha256 ?? "") && Number.isSafeInteger(evidence.archiveTocEntryCount) && evidence.archiveTocEntryCount >= 1 && evidence.archiveTocEntryCount <= 100000 && /^[0-9a-f]{64}$/.test(evidence.artifactVerificationDigest ?? "") &&
    typeof evidence.backupId === "string" && /^\d{8}T\d{6}Z-[0-9a-f]{32}$/.test(evidence.backupId) &&
