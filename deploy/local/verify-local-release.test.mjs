...
import { access, mkdtemp, mkdir, rm, truncate, writeFile } from "node:fs/promises";
...
test("package and verify cover the complete release tree and reject extras",async()=>{
  ...
  await writeFile(path.join(root,"api/node_modules/pkg/package.json"),JSON.stringify({name:"pkg",files:["index.js"],dependencies:{}}));await rm(path.join(root,"api/node_modules/pkg/index.js"));await assert.rejects(()=>verifyRuntimeClosure(root),/RELEASE_PACKAGE_ENTRYPOINT_MISSING:.*:\.\/index\.js/);
