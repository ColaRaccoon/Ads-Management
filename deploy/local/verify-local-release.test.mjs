...
import { inventory, verifyLocalRelease } from "./verify-local-release.mjs";
...
test("package and verify cover the complete release tree and reject extras",async()=>{
  ...
  for(const name of [
    ...
    "api/prisma/migrations/001_init/migration.sql","api/node_modules/pkg/index.js","api/node_modules/pkg/package.json",
    ...
    "web/.next/static/chunks/app.js","web/node_modules/pkg/index.js","web/node_modules/pkg/package.json",
  ...
  ]){const full=path.join(root,...name.split("/"));await mkdir(path.dirname(full),{recursive:true});const body=name.endsWith("package.json")?JSON.stringify({name:"fixture",dependencies:name==="api/package.json"||name==="web/package.json"?{pkg:"1.0.0"}:{}}):name.endsWith("required-server-files.json")?JSON.stringify({files:[".next/BUILD_ID",".next/server/app.js"]}):`fixture:${name}`;await writeFile(full,body);}
