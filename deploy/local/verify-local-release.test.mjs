...
test("package and verify cover the complete release tree and reject extras",async()=>{
  ...
  for(const name of [
    ...
    "api/dist/staging/mutation-compatibility-smoke.cli.js","api/prisma/migrations/001_init/migration.sql","api/node_modules/pkg/index.js","api/node_modules/pkg/cli.js","api/node_modules/pkg/native/addon.node","api/node_modules/pkg/package.json",
  ...
  for(const name of LOCAL_HOST_TOOL_FILES){const candidate=path.join(root,...name.split("/")),body=await readFile(candidate);await rm(candidate);await assert.rejects(()=>verifyRuntimeClosure(root),new RegExp(`RELEASE_REQUIRED_FILE_MISSING:${name.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")}`));await writeFile(candidate,body)}
  const unexpectedHost=path.join(root,"deploy/windows/unexpected.ps1");await writeFile(unexpectedHost,"# unexpected\n");await assert.rejects(()=>verifyRuntimeClosure(root),/RELEASE_FILE_NOT_ALLOWLISTED/);await rm(unexpectedHost);
  ...
  await writeFile(path.join(root,"unexpected.js"),"tamper");await assert.rejects(()=>verifyLocalRelease(root),/RELEASE_FILE_NOT_ALLOWLISTED/);
});
...
async function readFixtureVerifier(){return await (await import("node:fs/promises")).readFile(path.join(import.meta.dirname,"verify-local-release.mjs"),"utf8")}
function fixtureJavaScript(name){
  ...
  if(name==="api/dist/staging/mutation-compatibility-smoke.cli.js")return`process.stderr.write('{"event":"mutation-compatibility-smoke","result":"FAIL"}');process.exit(1);`;
...
async function createCompleteFixture(){
  ...
  for(const name of ["api/dist/main.js","api/dist/auth/bootstrap-local-super-admin.cli.js","api/dist/staging/business-compatibility-smoke.cli.js","api/dist/staging/legacy-business-compatibility-smoke.cli.js","api/dist/staging/auth-role-matrix-smoke.cli.js","api/dist/staging/mutation-compatibility-smoke.cli.js","api/package.json","api/prisma/schema.prisma","api/prisma/migrations/001_init/migration.sql","api/node_modules/pkg/index.js","api/node_modules/pkg/package.json","api/node_modules/@prisma/client/package.json","api/node_modules/@prisma/client/index.js","api/node_modules/.prisma/client/package.json","api/node_modules/.prisma/client/index.js","api/node_modules/.prisma/client/query_engine-windows.dll.node","api/node_modules/prisma/package.json","api/node_modules/prisma/index.js","api/node_modules/prisma/build/index.js","web/server.js","web/package.json","web/.next/BUILD_ID","web/.next/required-server-files.json","web/.next/server/app.js","web/.next/static/chunks/app.js","web/node_modules/pkg/index.js","web/node_modules/pkg/package.json",...LOCAL_HOST_TOOL_FILES,...WINDOWS_HOST_FILES]){
