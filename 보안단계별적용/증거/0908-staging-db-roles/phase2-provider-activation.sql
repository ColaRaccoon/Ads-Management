...
\echo G_DB_00_PHASE2_ACTIVATION_BEGIN
...
BEGIN
  IF current_database() <> 'postgres' OR current_user <> 'postgres' OR session_user <> 'postgres' THEN
    RAISE EXCEPTION 'R2A_PHASE2_PRE_PRINCIPAL_MISMATCH';
  END IF;
  IF current_setting('server_version_num')::integer / 10000 <> 17 THEN
    RAISE EXCEPTION 'R2A_PHASE2_PRE_POSTGRES_MAJOR_MISMATCH';
  END IF;
  IF current_setting('password_encryption') <> 'scram-sha-256' THEN
    RAISE EXCEPTION 'R2A_PHASE2_PRE_PASSWORD_ENCRYPTION_MISMATCH';
  END IF;
  IF current_setting('scram_iterations')::integer <> 4096 THEN
    RAISE EXCEPTION 'R2A_PHASE2_PRE_SCRAM_ITERATIONS_MISMATCH';
  END IF;
  IF md5(COALESCE((SELECT datacl::text FROM pg_database WHERE datname = 'postgres'), '<NULL>'))
    ...
    RAISE EXCEPTION 'R2A_PHASE2_PRE_BASE_ACL_MISMATCH';
  END IF;
  ...
  IF (SELECT count(*) FROM pg_roles
    ...
    RAISE EXCEPTION 'R2A_PHASE2_PRE_ROLE_ATTRIBUTE_OR_OID_MISMATCH';
  END IF;
  ...
  IF creator_edge_count <> 3 OR forbidden_membership_count <> 0 THEN
    RAISE EXCEPTION 'R2A_PHASE2_PRE_ROLE_MEMBERSHIP_MISMATCH';
  END IF;
  IF (SELECT count(DISTINCT granted_role.rolname)
    ...
    RAISE EXCEPTION 'R2A_PHASE2_PRE_ROLE_MEMBERSHIP_CARDINALITY_MISMATCH';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'R2A_PHASE2_PRE_DATABASE_ACL_MISMATCH';
  END IF;
  IF (SELECT count(*) FROM pg_stat_activity WHERE datname = 'meta_ads_staging') <> 0 THEN
    RAISE EXCEPTION 'R2A_PHASE2_PRE_TARGET_SESSION_PRESENT';
  END IF;
END
$phase2_precondition$;

-- The Windows runner derives three PostgreSQL-format SCRAM verifiers client-side with independent
...
ALTER ROLE meta_ads_stg_runtime LOGIN;
ALTER ROLE meta_ads_stg_migration LOGIN;
ALTER ROLE meta_ads_stg_backup LOGIN;

DO $phase2_postcondition$
BEGIN
  IF (SELECT count(*) FROM pg_roles
      WHERE (rolname = 'meta_ads_stg_runtime'
          AND oid = current_setting('r2a.expected_runtime_oid')::oid
        ...
        AND NOT rolcreaterole AND NOT rolreplication AND NOT rolbypassrls
        AND rolconnlimit = 2) <> 3 THEN
    RAISE EXCEPTION 'R2A_PHASE2_POST_ROLE_ATTRIBUTE_OR_OID_MISMATCH';
  END IF;
  IF md5(COALESCE((SELECT datacl::text FROM pg_database WHERE datname = 'postgres'), '<NULL>'))
      IS DISTINCT FROM current_setting('r2a.expected_base_acl_md5') THEN
    RAISE EXCEPTION 'R2A_PHASE2_POST_BASE_ACL_CHANGED';
  END IF;
  IF (SELECT count(DISTINCT granted_role.rolname)
      FROM pg_auth_members m
      JOIN pg_roles granted_role ON granted_role.oid = m.roleid
    ...
    RAISE EXCEPTION 'R2A_PHASE2_POST_ROLE_MEMBERSHIP_MISMATCH';
  END IF;
  IF (SELECT count(*) FROM pg_stat_activity WHERE datname = 'meta_ads_staging') <> 0 THEN
    RAISE EXCEPTION 'R2A_PHASE2_POST_TARGET_SESSION_PRESENT';
  END IF;
END
$phase2_postcondition$;

SELECT 'PHASE2_ACTIVATION_TX_READY' AS status;
COMMIT;

\echo G_DB_00_PHASE2_ACTIVATION_COMMITTED
