...
param(
  [Parameter(Mandatory)]
  [ValidateSet('RecoveryExecute')]
  [string]$Mode,
  [Parameter(Mandatory)]
  [ValidateSet('G_DB_00_PHASE2_CREDENTIAL_RECOVERY_EXECUTE_APPROVED_EXACT_V1')]
...
$clientImageDigest = $clientImage.Split('@', 2)[1]
...
$activationPath = Join-Path $PSScriptRoot 'phase2-provider-credential-recovery.sql'
...
if ($Mode -ne 'RecoveryExecute' -or
    $ApprovalToken -ne 'G_DB_00_PHASE2_CREDENTIAL_RECOVERY_EXECUTE_APPROVED_EXACT_V1') {
...
function Get-R2AExactAclFingerprint {
  param([Parameter(Mandatory)][string]$Path)
  $acl = Get-Acl -LiteralPath $Path
  return '{0}|{1}|{2}' -f $acl.Owner, $acl.AreAccessRulesProtected, $acl.Sddl
}
...
function Write-R2ARecoveryActivationMarkerCreateNew {
  param(
    [Parameter(Mandatory)][string]$Root,
    [Parameter(Mandatory)][string]$CandidateSetId
  )
  $path = Join-Path $Root 'phase2-recovery-activation.json'
  if (Test-Path -LiteralPath $path) { throw 'R2A_RECOVERY_MARKER_ALREADY_EXISTS' }
  $candidate = Join-Path $Root ('.phase2-recovery-activation.candidate.' + [Guid]::NewGuid().ToString('N'))
  $record = [ordered]@{
    version = 'r2a-staging-role-recovery-activation/v1'
    ...
    roleCount = 3
    verifiedRoleCount = 3
    finalAdminVerify = $true
    originalCredentialRecordsPreserved = $true
    activatedAtUtc = [DateTimeOffset]::UtcNow.ToString('o')
    operationalReady = $false
  }
  $json = ($record | ConvertTo-Json -Depth 6) + "`n"
  $expectedMarkerSha256 = [Convert]::ToHexString(
    [Security.Cryptography.SHA256]::HashData([Text.UTF8Encoding]::new($false).GetBytes($json))
  ).ToLowerInvariant()
  ...
  try {
    Set-R2AUserSystemOnlyAcl -Path $candidate -Directory $false
    Assert-R2AUserSystemOnlyAcl -Path $candidate
    Assert-R2ANotReparsePoint -Path $candidate
    if ((Get-R2AFileSha256 -Path $candidate) -ne $expectedMarkerSha256) {
      throw 'R2A_RECOVERY_MARKER_CANDIDATE_HASH_MISMATCH'
    }
    $candidateRecord = Get-Content -Raw -LiteralPath $candidate | ConvertFrom-Json
    if ($candidateRecord.version -ne $record.version -or $candidateRecord.status -ne 'active_verified' -or
        $candidateRecord.candidateSetId -ne $CandidateSetId -or $candidateRecord.verifiedRoleCount -ne 3 -or
        $candidateRecord.finalAdminVerify -ne $true -or $candidateRecord.operationalReady -ne $false) {
      throw 'R2A_RECOVERY_MARKER_CANDIDATE_SHAPE_MISMATCH'
    }
  } catch {
...
function Assert-R2AExecutionStateShape {
  ...
  $expectedTopKeys = @(
    ...
    'operationalReady', 'candidateSetId', 'sourceStatus', 'sourceCommitAckObserved',
    'sourceVerifiedRoleCount', 'sourceFinalAdminVerify', 'recoveryAttemptCount',
    'originalCredentialRecordsPreserved', 'candidateCredentialRecordsPreserved',
    'classificationReceiptSha256', 'sourceExecutionStateSha256'
  )
  ...
  foreach ($name in @(
      ...
      'database', 'phase1ReceiptSha256', 'candidateSetId', 'sourceStatus',
      'classificationReceiptSha256', 'sourceExecutionStateSha256'
    )) {
  ...
  foreach ($name in @(
      ...
      'negativeAuthTestCount', 'verifyOnlyAttemptCount', 'sourceVerifiedRoleCount',
      'recoveryAttemptCount'
    )) {
    if (-not ($Record[$name] -is [int]) -and -not ($Record[$name] -is [long])) {
  ...
  foreach ($name in @(
      ...
      'sourceCommitAckObserved', 'sourceFinalAdminVerify',
      'originalCredentialRecordsPreserved', 'candidateCredentialRecordsPreserved'
    )) {
  ...
  foreach ($name in @('runtime', 'migration', 'backup')) {
    if (-not ($Record.roleOids[$name] -is [int]) -and -not ($Record.roleOids[$name] -is [long])) {
  ...
  if ($Record.version -ne 'r2a-phase2-recovery-execution-state/v1' -or
      $Record.projectRef -ne 'ehnfrrmbkvlsbpvqcvkr' -or $Record.host -ne 'aws-0-ap-northeast-2.pooler.supabase.com' -or
      $Record.port -ne 5432 -or $Record.database -ne 'meta_ads_staging' -or $Record.databaseOid -ne 25404 -or
      $Record.roleOids.runtime -ne 25397 -or $Record.roleOids.migration -ne 25399 -or $Record.roleOids.backup -ne 25401 -or
      $Record.sourceStatus -ne 'COMMITTED_VERIFY_FAILED' -or $Record.sourceCommitAckObserved -ne $true -or
      $Record.sourceVerifiedRoleCount -ne 0 -or $Record.sourceFinalAdminVerify -ne $false -or
      $Record.recoveryAttemptCount -notin @(0,1) -or $Record.verifiedRoleCount -lt 0 -or $Record.verifiedRoleCount -gt 3 -or
      $Record.automaticRetryCount -ne 0 -or $Record.negativeAuthTestCount -ne 0 -or $Record.verifyOnlyAttemptCount -ne 0 -or
      $Record.originalCredentialRecordsPreserved -ne $true -or $Record.candidateCredentialRecordsPreserved -ne $true -or
      $Record.secretValuesOrHashesStored -ne $false -or $Record.operationalReady -ne $false) {
    throw 'R2A_RECOVERY_EXECUTION_STATE_SEMANTIC_MISMATCH'
  }
  $allowedStatuses = @(
    'RECOVERY_INTENT_RECORDED','RECOVERY_LOCAL_FAILURE_NO_PROVIDER_ATTEMPT',
    'RECOVERY_ATTEMPT_STARTED','RECOVERY_PRECOMMIT_SCRIPT_FAILED_ROLLBACK_EXPECTED',
    'RECOVERY_COMMIT_OUTCOME_UNKNOWN','RECOVERY_COMMIT_ACK_OBSERVED',
    'RECOVERY_COMMITTED_VERIFY_IN_PROGRESS','RECOVERY_COMMITTED_VERIFY_FAILED',
    'RECOVERY_PROVIDER_VERIFIED_PENDING_LOCAL_CLEANUP',
    'RECOVERY_PROVIDER_VERIFIED_LOCAL_CLEANUP_FAILED','RECOVERY_VERIFIED',
    'RECOVERY_VERIFIED_LOCAL_FINALIZE_FAILED'
  )
  if ($Record.status -notin $allowedStatuses) { throw 'R2A_RECOVERY_EXECUTION_STATE_STATUS_MISMATCH' }
  if (($Record.status -in @('RECOVERY_INTENT_RECORDED','RECOVERY_LOCAL_FAILURE_NO_PROVIDER_ATTEMPT') -and
        ($Record.recoveryAttemptCount -ne 0 -or $Record.commitAckObserved -or $Record.verifiedRoleCount -ne 0 -or $Record.finalAdminVerify)) -or
      ($Record.status -in @('RECOVERY_ATTEMPT_STARTED','RECOVERY_PRECOMMIT_SCRIPT_FAILED_ROLLBACK_EXPECTED','RECOVERY_COMMIT_OUTCOME_UNKNOWN') -and
        ($Record.recoveryAttemptCount -ne 1 -or $Record.commitAckObserved -or $Record.verifiedRoleCount -ne 0 -or $Record.finalAdminVerify)) -or
      ($Record.status -eq 'RECOVERY_COMMIT_ACK_OBSERVED' -and
        ($Record.recoveryAttemptCount -ne 1 -or -not $Record.commitAckObserved -or $Record.verifiedRoleCount -ne 0 -or $Record.finalAdminVerify)) -or
      ($Record.status -in @('RECOVERY_COMMITTED_VERIFY_IN_PROGRESS','RECOVERY_COMMITTED_VERIFY_FAILED') -and
        ($Record.recoveryAttemptCount -ne 1 -or -not $Record.commitAckObserved -or $Record.finalAdminVerify)) -or
      ($Record.status -in @('RECOVERY_PROVIDER_VERIFIED_PENDING_LOCAL_CLEANUP','RECOVERY_PROVIDER_VERIFIED_LOCAL_CLEANUP_FAILED','RECOVERY_VERIFIED','RECOVERY_VERIFIED_LOCAL_FINALIZE_FAILED') -and
        ($Record.recoveryAttemptCount -ne 1 -or -not $Record.commitAckObserved -or $Record.verifiedRoleCount -ne 3 -or -not $Record.finalAdminVerify))) {
    throw 'R2A_RECOVERY_EXECUTION_STATE_TRANSITION_MISMATCH'
  }
}
...
function Remove-R2AOwnedTempContainer {
  ...
  if ($OwnerLabel -notmatch '^r2a-phase2-recovery-[a-f0-9]{32}$') {
  ...
  if (-not (Test-Path -LiteralPath $CidFilePath -PathType Leaf)) {
    $ownedByName = Invoke-R2ADockerControl -Arguments @(
      ...
      '--filter', "name=^/$Name`$",
      '--filter', "label=com.colaraccoon.r2a.phase2.recovery.owner=$OwnerLabel"
    )
  ...
  $inspect = Invoke-R2ADockerControl -Arguments @(
    ...
    '--format', '{{.Id}}|{{index .Config.Labels "com.colaraccoon.r2a.phase2.recovery.owner"}}',
    $containerId
  )
...
function New-R2ABaseDockerArguments {
  param(
    ...
    [Parameter(Mandatory)][string]$CaHostPath,
  ...
  return @(
    ...
    '--cidfile', $CidFilePath,
    '--label', "com.colaraccoon.r2a.phase2.recovery.owner=$OwnerLabel",
    ...
    '--mount', "type=bind,source=$CaHostPath,target=/run/ca.crt,readonly",
...
$requiredExecutionHashes = [ordered]@{
  ...
  $activationPath = '2edb6432079fab1b89f3bd7fe492a161bc37c506d98a4862299616f827ed78c3'
...
$imageProbe = Invoke-R2ADockerControl -Arguments @(
  '--host', $dockerEndpoint, 'image', 'inspect', $clientImage,
  '--format', '{{.Os}}/{{.Architecture}}|{{join .RepoDigests ","}}'
)
if ($imageProbe.ExitCode -ne 0 -or
    -not $imageProbe.Stdout.Trim().StartsWith('linux/amd64|', [StringComparison]::Ordinal) -or
    -not $imageProbe.Stdout.Contains($clientImageDigest, [StringComparison]::Ordinal)) {
  throw 'R2A_PHASE2_PINNED_IMAGE_LOCAL_INSPECT_MISMATCH'
}
Assert-R2ANotReparsePoint -Path (Split-Path -Parent $caPath)
Assert-R2ANotReparsePoint -Path $caPath
Assert-R2AUserSystemOnlyAcl -Path $caPath
...
$originalSecretRoot = Get-R2ASecretRoot
$secretRoot = [IO.Path]::GetFullPath((Join-Path $env:APPDATA 'MetaAdsSecurity\staging-db-roles-recovery'))
$secretParent = [IO.Path]::GetFullPath((Split-Path -Parent $originalSecretRoot))
$sourceExecutionLockPath = Join-Path $secretParent 'staging-db-roles.phase2-recovery-source.lock'
$classificationReceiptPath = Join-Path (Split-Path -Parent $PSScriptRoot) '0909-g-db-00-phase2-readonly-classification.json'
$classificationReceiptSha256 = '807e0c52004beb235f2857077e42cc4e16951587dd3fcf2613f77b0d7ccdfe0d'
foreach ($path in @($originalSecretRoot, $secretRoot, $classificationReceiptPath)) { Assert-R2ANotReparsePoint -Path $path }
Assert-R2ANotReparsePoint -Path $secretParent
Assert-R2AUserSystemOnlyAcl -Path $secretParent
Assert-R2AUserSystemOnlyAcl -Path $originalSecretRoot
Assert-R2AUserSystemOnlyAcl -Path $secretRoot
$sourceExecutionLock = [IO.FileStream]::new(
...
Set-R2AUserSystemOnlyAcl -Path $sourceExecutionLockPath -Directory $false
Assert-R2AUserSystemOnlyAcl -Path $sourceExecutionLockPath
if ((Get-R2AFileSha256 -Path $classificationReceiptPath) -ne $classificationReceiptSha256) {
  throw 'R2A_RECOVERY_CLASSIFICATION_RECEIPT_HASH_MISMATCH'
}
$classification = Get-Content -Raw -LiteralPath $classificationReceiptPath | ConvertFrom-Json
if ($classification.status -ne 'CREDENTIAL_OR_VERIFIER_MISMATCH' -or
    $classification.classification.verifyOnlyAllowed -ne $false -or
    $classification.priorFailure.providerExecuteAttemptCount -ne 1 -or
    $classification.priorFailure.commitAckObserved -ne $true -or
    $classification.priorFailure.positiveRoleVerificationCount -ne 0 -or
    $classification.priorFailure.activeVerifiedMarkerPresent -ne $false) {
  throw 'R2A_RECOVERY_CLASSIFICATION_RECEIPT_CONTRACT_MISMATCH'
}
$sourceStatePath = Join-Path $originalSecretRoot 'phase2-execution-state.json'
$sourceMarkerPath = Join-Path $originalSecretRoot 'phase2-activation.json'
if (Test-Path -LiteralPath $sourceMarkerPath) { throw 'R2A_RECOVERY_SOURCE_MARKER_MUST_BE_ABSENT' }
Assert-R2ANotReparsePoint -Path $sourceStatePath
Assert-R2AUserSystemOnlyAcl -Path $sourceStatePath
$sourceState = Get-Content -Raw -LiteralPath $sourceStatePath | ConvertFrom-Json -AsHashtable -DateKind String
$sourceStateSha256 = Get-R2AFileSha256 -Path $sourceStatePath
if ($sourceState.version -ne 'r2a-phase2-execution-state/v1' -or
    $sourceState.status -ne 'COMMITTED_VERIFY_FAILED' -or
    $sourceState.projectRef -ne $target.ProjectRef -or $sourceState.database -ne $target.ApplicationDatabase -or
    $sourceState.databaseOid -ne 25404 -or $sourceState.roleOids.runtime -ne 25397 -or
    $sourceState.roleOids.migration -ne 25399 -or $sourceState.roleOids.backup -ne 25401 -or
    $sourceState.commitAckObserved -ne $true -or $sourceState.verifiedRoleCount -ne 0 -or
    $sourceState.finalAdminVerify -ne $false -or $sourceState.automaticRetryCount -ne 0 -or
    $sourceState.negativeAuthTestCount -ne 0 -or $sourceState.verifyOnlyAttemptCount -ne 0 -or
    $sourceState.secretValuesOrHashesStored -ne $false -or $sourceState.operationalReady -ne $false) {
  throw 'R2A_RECOVERY_SOURCE_STATE_CONTRACT_MISMATCH'
}
foreach ($role in $script:R2AAllowedRoles) { [void](Read-R2ACredentialRecord -Root $originalSecretRoot -Role $role) }
$expectedOriginalNames = @(
  'meta_ads_stg_backup.dpapi.json', 'meta_ads_stg_migration.dpapi.json',
  'meta_ads_stg_runtime.dpapi.json', 'phase2-execution-state.json'
)
$originalItems = @(Get-ChildItem -LiteralPath $originalSecretRoot -Force | Sort-Object Name)
if (($originalItems.Name -join "`n") -ne (($expectedOriginalNames | Sort-Object) -join "`n")) {
  throw 'R2A_RECOVERY_SOURCE_STORE_FILE_SET_MISMATCH'
}
$originalRootAclFingerprint = Get-R2AExactAclFingerprint -Path $originalSecretRoot
$originalTreeFingerprint = @($originalItems | ForEach-Object {
  $_.Name + ':' + $_.Length + ':' + (Get-R2AFileSha256 -Path $_.FullName) + ':' +
    (Get-R2AExactAclFingerprint -Path $_.FullName)
}) -join "`n"
$candidateSetPath = Join-Path $secretRoot 'recovery-candidate-set.json'
Assert-R2ANotReparsePoint -Path $candidateSetPath
Assert-R2AUserSystemOnlyAcl -Path $candidateSetPath
$candidateSet = Get-Content -Raw -LiteralPath $candidateSetPath | ConvertFrom-Json
if ($candidateSet.version -ne 'r2a-staging-role-recovery-candidate-set/v1' -or
    $candidateSet.status -ne 'PREPARED_NOT_ACTIVATED' -or
    $candidateSet.projectRef -ne $target.ProjectRef -or $candidateSet.database -ne $target.ApplicationDatabase -or
    [string]::IsNullOrWhiteSpace([string]$candidateSet.candidateSetId) -or
    [int]$candidateSet.roleCount -ne 3 -or $candidateSet.operationalReady -ne $false -or
    $candidateSet.secretValuesOrHashesStoredInMetadata -ne $false -or
    $candidateSet.originalExecutionStateSha256 -ne $sourceStateSha256 -or
    $candidateSet.classificationReceiptSha256 -ne $classificationReceiptSha256 -or
    $candidateSet.credentialsDifferFromAllOriginalAndCandidateValues -ne $true -or
    $candidateSet.activated -ne $false) {
  throw 'R2A_RECOVERY_CANDIDATE_SET_CONTRACT_MISMATCH'
}
$expectedRecoveryNames = @(
  'meta_ads_stg_backup.dpapi.json', 'meta_ads_stg_migration.dpapi.json',
  'meta_ads_stg_runtime.dpapi.json', 'recovery-candidate-set.json'
)
$actualRecoveryNames = @(Get-ChildItem -LiteralPath $secretRoot -Force | ForEach-Object { $_.Name } | Sort-Object)
if (($actualRecoveryNames -join "`n") -ne (($expectedRecoveryNames | Sort-Object) -join "`n")) {
  throw 'R2A_RECOVERY_CANDIDATE_STORE_FILE_SET_MISMATCH'
}
$activationMarkerPath = Join-Path $secretRoot 'phase2-recovery-activation.json'
$executionStatePath = Join-Path $secretRoot 'phase2-recovery-execution-state.json'
$executionLockPath = Join-Path $secretRoot 'phase2-recovery-execution.lock'
if (Test-Path -LiteralPath $activationMarkerPath) { throw 'R2A_RECOVERY_ALREADY_VERIFIED_NO_RETRY' }
if (Test-Path -LiteralPath $executionStatePath) { throw 'R2A_RECOVERY_PRIOR_EXECUTION_STATE_NO_RETRY' }
...
Set-R2AUserSystemOnlyAcl -Path $executionLockPath -Directory $false
  ...
  $inputLines = $null
$executionState = $null
$executionStage = 'RECOVERY_LOCAL_PRECONDITION'
try {
  $tempParent = [IO.Path]::GetFullPath((Join-Path $env:LOCALAPPDATA 'MetaAdsSecurity\phase2-recovery-temp'))
  ...
  $executionState = [ordered]@{
      version = 'r2a-phase2-recovery-execution-state/v1'
      ...
      sourceExecutionStateSha256 = $sourceStateSha256
      candidateSetId = [string]$candidateSet.candidateSetId
      sourceStatus = 'COMMITTED_VERIFY_FAILED'
      sourceCommitAckObserved = $true
      sourceVerifiedRoleCount = 0
      sourceFinalAdminVerify = $false
      recoveryAttemptCount = 0
      originalCredentialRecordsPreserved = $true
      candidateCredentialRecordsPreserved = $true
  ...
  $executionStage = 'RECOVERY_INTENT_RECORDED'

  $tempRoot = [IO.Path]::GetFullPath((Join-Path $tempParent ([Guid]::NewGuid().ToString('N'))))
  if (-not [IO.Path]::GetDirectoryName($tempRoot).Equals($tempParent,[StringComparison]::OrdinalIgnoreCase)) {
    throw 'R2A_RECOVERY_TEMP_SCOPE_INVALID'
  }
  [void][IO.Directory]::CreateDirectory($tempRoot)
  Set-R2AUserSystemOnlyAcl -Path $tempRoot -Directory $true
  $protectedCaPath = Join-Path $tempRoot 'official-ca.crt'
  $caBytes = [IO.File]::ReadAllBytes($caPath)
  try {
    $caBytesSha256 = [Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($caBytes)).ToLowerInvariant()
    if ($caBytesSha256 -ne $caSha256) { throw 'R2A_PHASE2_CA_COPY_SOURCE_HASH_MISMATCH' }
    [IO.File]::WriteAllBytes($protectedCaPath, $caBytes)
  } finally {
    [Array]::Clear($caBytes, 0, $caBytes.Length)
  }
  Set-R2AUserSystemOnlyAcl -Path $protectedCaPath -Directory $false
  Assert-R2AUserSystemOnlyAcl -Path $protectedCaPath
  if ((Get-R2AFileSha256 -Path $protectedCaPath) -ne $caSha256) {
    throw 'R2A_PHASE2_CA_PROTECTED_COPY_HASH_MISMATCH'
  }
  $containerRecords = [Collections.Generic.List[object]]::new()
  $ownerLabel = "r2a-phase2-recovery-$([Guid]::NewGuid().ToString('N'))"
  try {
    ...
    if ($Mode -eq 'RecoveryExecute') {
      $activationContainerName = "r2a-phase2-recovery-$suffix"
      ...
      $activationArgs = New-R2ABaseDockerArguments -Name $activationContainerName `
        ...
        -PgPassPath $adminPassPath -CaHostPath $protectedCaPath -SqlHostPath $activationPath
      $activationArgs += @(
      ...
      $executionStage = 'RECOVERY_ATTEMPT_STARTED'
      $executionState.status = 'RECOVERY_ATTEMPT_STARTED'
      $executionState.recoveryAttemptCount = 1
      ...
      [void](Invoke-R2ADockerPsql -DockerArguments $activationArgs `
        ...
        -ExpectedMarker 'G_DB_00_PHASE2_CREDENTIAL_RECOVERY_COMMITTED')
      $verifiers.Clear()
      $inputLines = $null
      $executionStage = 'RECOVERY_COMMIT_ACK_OBSERVED'
      $executionState.status = 'RECOVERY_COMMIT_ACK_OBSERVED'
    ...
    foreach ($role in $script:R2AAllowedRoles) {
      $externalUser = "$role.$($target.ProjectRef)"
      $roleContainerName = "r2a-phase2-recovery-$($role.Replace('meta_ads_stg_', ''))-$suffix"
      ...
      $roleArgs = New-R2ABaseDockerArguments -Name $roleContainerName `
        ...
        -PgPassPath $rolePassPaths[$role] -CaHostPath $protectedCaPath -SqlHostPath $roleVerifyPath
      $roleArgs += @(
        '-X', '-n', '-q', '-w', '-v', 'ON_ERROR_STOP=1', '-v', 'VERBOSITY=sqlstate',
      ...
      $executionState.status = 'RECOVERY_COMMITTED_VERIFY_IN_PROGRESS'
      $executionState.updatedAtUtc = [DateTimeOffset]::UtcNow.ToString('o')
      Write-R2AExecutionState -Path $executionStatePath -Record $executionState
    }

    $finalContainerName = "r2a-phase2-recovery-final-$suffix"
    ...
    $finalArgs = New-R2ABaseDockerArguments -Name $finalContainerName `
      ...
      -PgPassPath $adminPassPath -CaHostPath $protectedCaPath -SqlHostPath $finalVerifyPath
    $finalArgs += @(
      '-X', '-n', '-q', '-w', '-v', 'ON_ERROR_STOP=1', '-v', 'VERBOSITY=sqlstate',
      '-v', "expected_database_oid=$($target.DatabaseOid)",
      '-v', "expected_runtime_oid=$($target.RuntimeOid)",
    ...
    $executionStage = 'RECOVERY_PROVIDER_VERIFIED_PENDING_LOCAL_CLEANUP'
    $executionState.status = 'RECOVERY_PROVIDER_VERIFIED_PENDING_LOCAL_CLEANUP'
    $executionState.finalAdminVerify = $true
    $executionState.updatedAtUtc = [DateTimeOffset]::UtcNow.ToString('o')
    Write-R2AExecutionState -Path $executionStatePath -Record $executionState
    $successSummary = [ordered]@{
      status = 'PHASE2_CREDENTIAL_RECOVERY_AND_THREE_ROLE_VERIFY_PASS'
      mode = $Mode
      target = [ordered]@{
        projectRef = $target.ProjectRef
        host = $target.Host
      ...
      credentialRecoveryCommitted = $true
      credentialRecoveryAttemptedThisRun = $true
      positiveRoleVerificationCount = $roleResults.Count
      requireAuth = 'scram-sha-256'
      sslMode = 'verify-full'
      credentialState = 'recovery_active_verified_marker_written'
      originalCredentialRecordsPreserved = $true
      candidateCredentialRecordsPreserved = $true
  ...
  Assert-R2AUserSystemOnlyAcl -Path $originalSecretRoot
  if ((Get-R2AExactAclFingerprint -Path $originalSecretRoot) -cne $originalRootAclFingerprint) {
    throw 'R2A_RECOVERY_SOURCE_ROOT_ACL_CHANGED'
  }
  $sourceAfter = @(Get-ChildItem -LiteralPath $originalSecretRoot -Force | Sort-Object Name)
  foreach ($item in $sourceAfter) { Assert-R2AUserSystemOnlyAcl -Path $item.FullName }
  $sourceAfterFingerprint = @($sourceAfter | ForEach-Object {
    $_.Name + ':' + $_.Length + ':' + (Get-R2AFileSha256 -Path $_.FullName) + ':' +
      (Get-R2AExactAclFingerprint -Path $_.FullName)
  }) -join "`n"
  if ($sourceAfterFingerprint -cne $originalTreeFingerprint) {
    throw 'R2A_RECOVERY_SOURCE_STORE_CHANGED'
  }
  $executionStage = 'RECOVERY_VERIFIED'
  $executionState.status = 'RECOVERY_VERIFIED'
  $executionState.updatedAtUtc = [DateTimeOffset]::UtcNow.ToString('o')
  Write-R2AExecutionState -Path $executionStatePath -Record $executionState
  Write-R2ARecoveryActivationMarkerCreateNew -Root $secretRoot -CandidateSetId ([string]$candidateSet.candidateSetId)
  $executionStage = 'RECOVERY_MARKER_PUBLISHED'
  $successSummary | ConvertTo-Json -Depth 5
} catch {
  if ($null -ne $executionState -and (Test-Path -LiteralPath $executionStatePath)) {
    if ($executionStage -eq 'RECOVERY_LOCAL_PRECONDITION' -or $executionStage -eq 'RECOVERY_INTENT_RECORDED') {
      $executionState.status = 'RECOVERY_LOCAL_FAILURE_NO_PROVIDER_ATTEMPT'
    } elseif ($executionStage -eq 'RECOVERY_ATTEMPT_STARTED') {
      if ($_.Exception.Message -match 'FAILED_EXIT_3_') {
        $executionState.status = 'RECOVERY_PRECOMMIT_SCRIPT_FAILED_ROLLBACK_EXPECTED'
      } else {
        $executionState.status = 'RECOVERY_COMMIT_OUTCOME_UNKNOWN'
      }
    } elseif ($executionStage -eq 'RECOVERY_COMMIT_ACK_OBSERVED') {
      $executionState.status = 'RECOVERY_COMMITTED_VERIFY_FAILED'
    } elseif ($executionStage -eq 'RECOVERY_PROVIDER_VERIFIED_PENDING_LOCAL_CLEANUP') {
      $executionState.status = 'RECOVERY_PROVIDER_VERIFIED_LOCAL_CLEANUP_FAILED'
    } elseif ($executionStage -eq 'RECOVERY_VERIFIED') {
      $executionState.status = 'RECOVERY_VERIFIED_LOCAL_FINALIZE_FAILED'
    }
    $executionState.updatedAtUtc = [DateTimeOffset]::UtcNow.ToString('o')
  ...
  if ($null -ne $verifiers) { $verifiers.Clear() }
  $inputLines = $null
  $secrets.Clear()
  $executionLock.Dispose()
  $sourceExecutionLock.Dispose()
}
