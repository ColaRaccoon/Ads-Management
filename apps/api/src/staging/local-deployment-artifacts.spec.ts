...
describe("local native deployment artifacts", () => {
  ...
  it("uses release manifest v4 and signed mode-specific quiescence across compatibility, migration, and rollback",async()=>{
    const[compatibility,initial,restore,migration,rollback,approval,mutationSmoke]=await Promise.all([file("deploy/windows/Test-ReleaseCompatibility.ps1"),file("deploy/windows/Test-InitialCutoverCompatibility.ps1"),file("deploy/windows/Restore-Verify.ps1"),file("deploy/windows/Manage-SupabaseMigration.ps1"),file("deploy/windows/Manage-SupabaseRollbackRestore.ps1"),file("deploy/windows/approval-plan.ps1"),file("apps/api/src/staging/mutation-compatibility-smoke.cli.ts")]);
