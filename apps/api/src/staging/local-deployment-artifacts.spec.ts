...
describe("local native deployment artifacts", () => {
  ...
  it("uses release manifest v4 and signed mode-specific quiescence across compatibility, migration, and rollback",async()=>{
    ...
    for(const token of["contractVersion:MUTATION_COMPATIBILITY_CONTRACT_VERSION","databaseMutations:10","storageMutations:10"]){expect(mutationSmoke).toContain(token)}
    for(const token of["contractVersion-ne4","flowCount-ne9","databaseMutations-ne10","storageMutations-ne10","replaceManualPurchasesForDate+deleteManualPurchase","content-canonical-and-hash-verified","reportContentDigest-notmatch","reportContentVerified","purgeStoredObject:db-hash-and-byte-absence-verified"]){expect(restore).toContain(token)}
    for(const source of[compatibility,initial,restore])expect(source).toContain("mutationCompatibilityContractVersion=4");
  ...
  it("bounds every migration child and leaves a fail-closed maintenance journal",async()=>{
    ...
    expect(migration).toContain("[Diagnostics.ProcessStartInfo]::new()");expect(migration).toContain("BaseStream.ReadAsync");expect(migration).toContain("Stop-VerifiedRecoveryProcessTree");expect(migration).toContain("ExpectedRecoveryProcessTreeHelperSha256");expect(migration).toContain("processTreeExitVerified=$true");
  ...
  it("supports source-independent authenticated disaster extraction and an enabled plan-bound daily trigger",async()=>{
    ...
    for(const token of["CREATE_SUSPENDED","AssignProcessToJobObject","JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE","PROC_THREAD_ATTRIBUTE_HANDLE_LIST","TerminateJobObject","ActiveProcessCount","RECOVERY_PROCESS_NOT_JOB_BOUND"]){expect(recoveryTree).toContain(token)}
    expect(recoveryTree.indexOf("AssignProcessToJobObject")).toBeLessThan(recoveryTree.indexOf("ResumeThread(pi.hThread)"));
    if(process.platform==="win32"){const processTreeTest=spawnSync("pwsh.exe",["-NoProfile","-ExecutionPolicy","Bypass","-File",path.join(root,"deploy/windows/recovery-process-tree.runtime.test.ps1")],{cwd:root,encoding:"utf8",timeout:20_000});expect(processTreeTest.status,processTreeTest.stderr).toBe(0);const value=JSON.parse(processTreeTest.stdout);expect(value.stubbornDescendantTerminated).toBe(true);expect(value.spawnDuringTerminationContained).toBe(true);expect(value.suspendedBeforeJobAssignment).toBe(true);expect(value.jobActiveProcessCountZero).toBe(true)}
    expect(recovery.indexOf("foreach($purpose in @('edge-signing','backup-receipt','restore-receipt'))")).toBeLessThan(recovery.indexOf("return Get-InventoryDigest $Manifest.files"));expect(recovery).toContain("ManifestKeyPairInventoryDigest=ShaText");
  ...
  it("preserves the live schema behind a durable rollback journal and rejects local NAS aliases",async()=>{
    ...
    for(const token of["DAILY_BACKUP_RECOVERY","STOPPED_LOCAL_SERVICES_ABSENCE_V2","ROLLBACK_DAILY_RECOVERY_REHEARSAL_REJECTED","ROLLBACK_DAILY_JOURNAL_WORKSPACE_BINDING_REQUIRED","ROLLBACK_RECOVERY_WORKSPACE_TOPOLOGY_REJECTED","ROLLBACK_RECOVERY_ORIGINAL_STORAGE_NOT_FOUND","RestoreRehearsalEvidencePath","RecoveryWorkspaceRoot","ExpectedLocalRecoverySecurityHelperSha256","Assert-ActionTimeRecoveryBoundary","Assert-DailyRecoveryAclState","Prepare-DailyRecoveryAclForRecover","Test-DailyRecoveryJournalBinding","RECOVER_NORMALIZED","TRANSITIONAL","CORE_MODIFY","ADMIN_ONLY","Set-LocalRecoveryExactAcl","productionDatabaseMutated","Copy-VerifiedStoragePayload","Restore-OriginalStorage","productionStorageRestored","originalStorageInventoryDigest","rpoHours-ne24","rtoHours-ne4"]){expect(rollback).toContain(token)}
    for(const token of["LOCAL_RECOVERY_CORE_SERVICE_NOT_DISABLED_AND_STOPPED","LOCAL_RECOVERY_EDGE_SERVICE_NOT_DISABLED_AND_STOPPED","LOCAL_RECOVERY_PROTECTED_LISTENER_PRESENT","LOCAL_RECOVERY_SERVICE_ACCOUNT_PROCESS_PRESENT","LOCAL_RECOVERY_CORE_RIGHTS_MISMATCH","LOCAL_RECOVERY_ACL_TRANSITION_DESCRIPTOR_REJECTED","Assert-LocalRecoveryExactAcl","Assert-LocalRecoveryAclOneOf","Set-LocalRecoveryExactAcl"]){expect(recoverySecurity).toContain(token)}
    if(process.platform==="win32"){const recoverySecurityTest=spawnSync("pwsh.exe",["-NoProfile","-ExecutionPolicy","Bypass","-File",path.join(root,"deploy/windows/local-recovery-security.runtime.test.ps1")],{cwd:root,encoding:"utf8",timeout:20_000});expect(recoverySecurityTest.status,recoverySecurityTest.stderr).toBe(0);const value=JSON.parse(recoverySecurityTest.stdout);expect(value.orphanProcessRejected).toBe(true);expect(value.rightsDriftRejected).toBe(true)}
    expect(rollback.indexOf("Write-AtomicJson $RollbackJournalPath $intent -CreateOnly")).toBeLessThan(rollback.indexOf("Copy-VerifiedStoragePayload $backupPayloadRoot $stagingStorageRoot"));
  });
  ...
  it("separates pre-migration resume from signed post-migration rollback and provides an explicit preserved-schema finalizer",async()=>{
    ...
    for(const token of["RETURN_FORWARD","ACCEPT_ROLLBACK_DROP_PRESERVED","Assert-ApprovedPlan","supabase_postgres","database.migrationUser","COMPLETE_MAINTENANCE_REQUIRED","DROP SCHEMA %I CASCADE","ALTER SCHEMA %I RENAME TO %I","Invoke-AtomicFinalization","ROLLBACK_FINALIZE_ATOMIC_TRANSACTION_FAILED"]){expect(finalizer).toContain(token)}
    ...
    for(const token of["Get-LiveSchemaFingerprint","live-schema-fingerprint-v3","relrowsecurity","relforcerowsecurity","pg_policy","polwithcheck","to_jsonb(t)::text","FROM ONLY %I.%I","pg_advisory_xact_lock","LOCK TABLE %I.%I IN ACCESS EXCLUSIVE MODE","initialCurrentSchemaFingerprint","initialPreservedSchemaFingerprint","expectedSurvivorSchemaFingerprint","ROLLBACK_FINALIZE_RECONCILE_FINGERPRINT_DRIFT","ROLLBACK_FINALIZE_SURVIVOR_FINGERPRINT_DRIFT","ROLLBACK_FINALIZE_PRE_MUTATION_MAINTENANCE_HASH_DRIFT","catalogFingerprintVerified=$true","rowLevelSecurityFingerprintVerified=$true","policyFingerprintVerified=$true","migrationChainFingerprintVerified=$true","businessKpiFingerprintVerified=$true","storageReferenceFingerprintVerified=$true","fullApplicationRowFingerprintVerified=$true","singleLockedDatabaseTransactionVerified=$true","ExpectedRecoveryProcessTreeHelperSha256","BaseStream.ReadAsync"]){expect(finalizer).toContain(token)}
    for(const token of["DAILY_BACKUP_RECOVERY","STOPPED_LOCAL_EDGE","STOPPED_LOCAL_SERVICES_ABSENCE_V2","RecoveryWorkspaceRoot","ExpectedLocalRecoverySecurityHelperSha256","CORE_MODIFY","ADMIN_ONLY","Get-StorageInventoryDigest","preservedOriginalStorageRetained","ROLLBACK_FINALIZE_DAILY_STORAGE_DRIFT","ROLLBACK_FINALIZE_DAILY_RETURN_REQUIRES_RECOVER_ACTION"]){expect(finalizer).toContain(token)}
    expect(finalizer).not.toContain("ReadToEndAsync");expect(finalizer.indexOf("Write-DurableJson $FinalizationJournalPath $intent -CreateOnly")).toBeLessThan(finalizer.indexOf("[void](Invoke-AtomicFinalization"));
    ...
    expect(finalizer.indexOf("Write-DurableJson $FinalizationJournalPath $intent -CreateOnly")).toBeLessThan(finalizer.indexOf("[void](Invoke-AtomicFinalization"));
