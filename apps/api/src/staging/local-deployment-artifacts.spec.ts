...
describe("local native deployment artifacts", () => {
  ...
  it("uses release manifest v4 and signed mode-specific quiescence across compatibility, migration, and rollback",async()=>{
    ...
    for(const token of["contractVersion-ne3","flowCount-ne9","databaseMutations-ne10","storageMutations-ne10","replaceManualPurchasesForDate+deleteManualPurchase","purgeStoredObject:db-hash-and-byte-absence-verified"]){expect(restore).toContain(token)}
    for(const source of[compatibility,initial,restore])expect(source).toContain("mutationCompatibilityContractVersion=3");
  ...
  it("preserves the live schema behind a durable rollback journal and rejects local NAS aliases",async()=>{
    ...
    for(const token of["DAILY_BACKUP_RECOVERY","STOPPED_LOCAL_EDGE_ABSENCE_V1","ROLLBACK_DAILY_RECOVERY_REHEARSAL_REJECTED","RestoreRehearsalEvidencePath","productionDatabaseMutated","Copy-VerifiedStoragePayload","Restore-OriginalStorage","productionStorageRestored","originalStorageInventoryDigest","rpoHours-ne24","rtoHours-ne4"]){expect(rollback).toContain(token)}
  ...
  it("separates pre-migration resume from signed post-migration rollback and provides an explicit preserved-schema finalizer",async()=>{
    ...
    for(const token of["DAILY_BACKUP_RECOVERY","STOPPED_LOCAL_EDGE","STOPPED_LOCAL_EDGE_ABSENCE_V1","Get-StorageInventoryDigest","preservedOriginalStorageRetained","ROLLBACK_FINALIZE_DAILY_STORAGE_DRIFT","ROLLBACK_FINALIZE_DAILY_RETURN_REQUIRES_RECOVER_ACTION"]){expect(finalizer).toContain(token)}
