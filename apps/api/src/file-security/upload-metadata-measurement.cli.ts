...
async function main() {
  ...
  const result = await measureExplicitUploadMetadata(parsed.inputs, { approvedRoot: parsed.approvedRoot });
...
function parseArguments(args: string[]) {
  ...
  if (!approvedRoot || inputs.length === 0) throw new Error("METADATA_ARGUMENT_INVALID");
  return { approvedRoot, inputs };
}
