...
describe("AuthenticationGuard", () => {
  it("rejects a request without the access cookie", async () => {
    ...
    await expect(guard.canActivate(contextFor({}) as ExecutionContext)).rejects.toMatchObject({
  ...
  it("attaches the DB-resolved principal to the request", async () => {
    ...
    await expect(guard.canActivate(contextFor(request) as ExecutionContext)).resolves.toBe(true);
