...
export default function DashboardPage() {
  ...
  const summary = useQuery({
    ...
    queryFn: () => apiGet<DashboardSummary>(`/dashboard/summary?${rangeQuery(range)}`)
  });
  const trends = useQuery({
    queryKey: ["dashboard-trends", range],
    queryFn: () => apiGet<Array<Record<string, unknown>>>(`/dashboard/trends?${rangeQuery(range)}`)
  });
  ...
  const stages = useQuery({
    queryKey: ["stage-trends", range],
    queryFn: () => apiGet<Array<Record<string, any>>>(`/dashboard/trends?${rangeQuery(range)}&groupBy=stage`)
  });
  const runDecision = useMutation({
    mutationFn: () => apiPost("/decisions/run", range),
  ...
  return (
    <section className="page">
      ...
      <div className="grid kpi">
        <KpiCard label="총 광고비" value={money(data?.totals.spendKrw)} helper={`${money(data?.totals.spendUsd, "USD")} 원본`} />
        <KpiCard label="총 구매수" value={numberFmt(data?.totals.purchaseCount)} helper={`${data?.selectedPeriod.dataDays ?? 0} data days`} />
        <KpiCard label="누적 CPA" value={money(data?.totals.cpaKrw)} helper="기간 합계 기반" />
        <KpiCard label="총 매출" value={money(data?.totals.revenueKrw)} />
        <KpiCard label="총 마진" value={money(data?.totals.marginKrw)} />
      </div>
