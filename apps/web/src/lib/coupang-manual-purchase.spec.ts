...
describe("Coupang manual-purchase draft", () => {
  it("excludes a blank or zero quantity only when there is no memo", () => {
    expect(parseSelectedManualPurchaseQuantity("", "상품")).toBeNull();
    expect(parseSelectedManualPurchaseQuantity("0", "상품")).toBeNull();
    expect(parseSelectedManualPurchaseQuantity("", "상품", "리뷰 보강")).toBe(0);
    expect(parseSelectedManualPurchaseQuantity("0", "상품", "리뷰 보강")).toBe(0);
  ...
  it("rejects a populated non-integer instead of dropping the replacement row", () => {
    expect(() => parseSelectedManualPurchaseQuantity("1.5", "기존 상품")).toThrow("수량은 0 이상의 정수");
    expect(() => parseSelectedManualPurchaseQuantity("-1", "기존 상품")).toThrow("수량은 0 이상의 정수");
  });
