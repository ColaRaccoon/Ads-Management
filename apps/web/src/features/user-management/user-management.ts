...
export function parseUserSummary(value: unknown): UserSummary {
  ...
  const summary: UserSummary = {
    ...
    reconciliationActions: reconciliationActions(value.reconciliationActions),
...
function auditFieldLabel(field: (typeof SAFE_AUDIT_FIELDS)[number]) {
  return { name: "이름", role: "역할", isActive: "계정", inviteStatus: "초대 상태" }[field];
}
...
export function invitationErrorMessage(error: unknown): string {
  ...
  const messages: Record<string, string> = {
    ...
    IDEMPOTENCY_KEY_INVALID: "초대 요청 식별자를 만들지 못했습니다. 입력을 다시 확인해 주세요.",
    IDEMPOTENCY_KEY_CONFLICT: "같은 요청 식별자가 다른 초대 내용에 사용되었습니다. 입력을 다시 확인해 주세요.",
    INVITATION_PROVIDER_UNAVAILABLE: "초대 제공자와 통신하지 못했습니다. 같은 요청으로 다시 시도하거나 조정 상태를 확인해 주세요.",
    INVITATION_NOT_RECONCILABLE: "현재 초대 상태에서는 이 조정 작업을 수행할 수 없습니다.",
    INVITATION_LOCAL_COMMIT_FAILED: "초대 제공자 처리 뒤 로컬 반영에 실패했습니다. 자동 활성화하지 않았으므로 목록의 조정 필요 상태를 확인해 주세요.",
...
export function invitationAcceptanceError(error: unknown): { message: string; retryable: boolean; activeSession: boolean } {
  ...
  if (code === "ACTIVE_SESSION_PRESENT") {
    return {
      message: "다른 계정으로 이미 로그인되어 있어 초대를 수락하지 않았습니다. 현재 계정에서 로그아웃하거나 시크릿 창에서 링크를 다시 여세요.",
      retryable: true,
      activeSession: true
    };
  }
  ...
  return {
    message: code === "INVITATION_INVALID_OR_EXPIRED"
      ? "초대 링크가 만료되었거나 이미 사용되었습니다. 총관리자에게 새 초대를 요청해 주세요."
      : "초대를 확인하지 못했습니다. 잠시 후 다시 시도하거나 총관리자에게 문의해 주세요.",
...
export function passwordErrorMessage(error: unknown): string {
  ...
  if (code === "ONBOARDING_SESSION_REQUIRED") return "초대 설정 세션이 만료되었습니다. 총관리자에게 새 초대를 요청해 주세요.";
  if (code === "PASSWORD_PROVIDER_UNAVAILABLE") return "비밀번호 제공자와 통신하지 못했습니다. 잠시 후 다시 시도해 주세요.";
  if (code === "PASSWORD_LOCAL_COMMIT_FAILED") return "비밀번호 제공자 처리 뒤 계정 활성화에 실패했습니다. 업무 접근은 계속 차단되어 있습니다. 같은 화면에서 다시 시도하거나 총관리자에게 문의해 주세요.";
